OZEETY Core version *3.1.0* is now available from:  <https://github.com/ozeety-project/ozeety/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/ozeety-project/ozeety/issues>


Mandatory Update
==============

OZEETY Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
eZEE Staking is here! eZEE staking will be activated on the morning of the 8th of May 2018. With the release of eZEE staking, there are effectively 2 versions of eZEE, eZEE minted on the 3.0.6 OZEETY wallet or lower, and eZEE minted on OZEETY wallet version or higher. New features in this release will require the use of eZEE v2, eZEE minted on this wallet release 3.1.0 or later. If you currently hold eZEE v1 and wish to take advantage of eZEE staking and deterministic eZEE, you will need to spend the eZEE v1 to yourself and remint eZEE v2.
Note: To find your eZEE version, click the privacy tab, then the eZEE Control button then expand the arrows next to the desired denomination.


### Deterministic eZEE Seed Keys

eZEE is now associated with a deterministic seed key. With this seed key, users are able to securely backup their eZEE outside of the wallet that the eZEE had been minted on. eZEE can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated eZEE minting

eZEE minting now only requires 1 further mint (down from 2) to mature. eZEE mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### eZEE Search

Users will now have the ability to search the blockchain for a specific serial # to see if a eZEE denomination has been spent or not.



OZTG/eZEE Staking and Masternode Rewards
--------------

### OZTG, eZEE and Masternode Payment Schedule

To encourage the use of eZEE and increase the OZEETY eshadow anonymity set, the OZEETY payment schedule has been changed to the following:

If a user staking eZEE wins the reward for their block, the following eZEE reward will be: 
- 3 eZEE (3 x 1 denominations) rewarded to the staker, 2 OZTG rewarded to the masternode owner and 1 OZTG available for the budget. This is a total block reward of 6 OZTG, up from 5.

If a user staking OZTG wins the reward, the following amounts will be rewarded: 
- 2 OZTG to the OZTG staker, 3 OZTG to the Masternode owner and 1 OZTG available for the budget. This is a total block reward of 6 OZTG, up from 5.


### Return change to sender when minting eZEE

Previously, eZEE minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most OZTG and return the change from a eZEE mint to that address.
...
The visual layout of the OZEETY Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto eZEE minting feature. This is enabled by default and the enablezeromint=0 setting in the ozeety.conf file will overwrite the GUI option.
-	The percentage of autominted eZEE can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular OZTG.
...
### Summary
 ...
 The OZEETY wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the ozeety.conf file, found in the OZEETY data directory.
- backuppath = <directory / full path>
- ezeebackuppath = <directory / full path>
...
* If eZEE backup, auto generated name is `wallet-autoezeebackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
OZEETY Daemon & Client (RPC Changes)
--------------
...
This allows running ozeetyd without having to do any manual configuration.
...
Search the eshadow database for a eshadowspend transaction that contains the given serial. This will be a helpful tool for the OZEETY support group, which often times sees users say "I didn't spend that eZEE". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the OZEETY masternodes, the monthly budget available to be utilised has been increased to 42,000 OZTG / month. This OZTG only has the opportunity to be raised once per month (paid to winning proposals) with any unused OZTG not created by the blockchain.

### Budget Finalization Fee

The OZEETY finalization fee for successful proposals has now been reduced, this fee is now 5 OZTG down from 50 OZTG. The total fee outlay for a successful proposal is now a total of 55 OZTG.
...
-	Enforced v1 eZEE spends to require a security level of 100
-	Updates to eZEE spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the OZEETY-Qt masternodes tab
-	Updated OZEETY-Qt tooltips
...
Further Reading: Version 2 Eshadows
==============

Several critical security flaws in the eshadow protocol and OZEETY's eshadow implementation have been patched. Enough has changed that new eshadows are distinct from old eshadows, and have been labelled as *version 2*. When using the eZEE Control dialog in the QT wallet, a user is able to see eZEE marked as version 1 or 2.

zPoS (eZEE staking)
--------------

Once a eZEE has over 200 confirmations it becomes available to stake. Staking eZEE will consume the exact eshadow that is staked and replace it with a freshly minted eshadow of the same denomination as well as a reward of three 1 denomination eZEE. So for example if a 1,000 eZEE denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination eZEEs.

Secure Spending
--------------

Version 1 eshadows, as implemented by [Miers et. al](http://eshadow.org/media/pdf/EshadowOakland.pdf), allow for something we describe as *serial trolling*. Spending eshadows requires that the spender reveal their serial number associated with the eshadow, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within OZEETY's eshadow implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new eshadow with the same serial number, and spend the new eshadow before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.

The serial troll situation is mitigated in version 2 by requiring that the serial number be a hash of a public key. The spend requires an additional signature signed by the private key associated with the public key hash matching the serial number. This work around was conceived by Tim Ruffing, a cryptographer that has studied the eshadow protocol and done consulting work for the ZCoin project.

Deterministic Eshadow Generation
--------------

Eshadows, or eZEE, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of eZEE that can be recalculated at any time using the seed. Deterministic eZEE allows for users to backup all of their future eZEE by simply recording their seed and keeping it in a safe place (similar to backing up a private key for OZTG). The eZEE seed needs to remain in the wallet in order to spend the eZEE after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private eZEE data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Eshadow Modulus
--------------

OZEETY's eshadow implementation used the same code from the ZCoin project to import the modulus use for the eshadow protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which OZEETY used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to OZEETY using the bug bounty program. The modulus is now correctly imported and OZEETY's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] eZEE v2: zPoS deZEE eeZEE external backup and budget fixes (too many to list)
 - #558 `c7e6f0f7f` [Core] Remove Gitan-OSX warning for High Sierra builds (Mrs-X)
 - #523 `926c073ea` [Core] Give high priority to eshadowspends to make it into the next block. (presstab)
 ...
 - #428 `3de6b6f3e` [Core] Wipe eshadow DB on reindex. (presstab)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh eZEE balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of eZEE status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "eZee Selected" labels after sending. (presstab)
...
### RPC/REST
 - #562 `772160b1b` [Wallet/RPC] Add argument to mint eshadow from specific UTXO (warrows)
 - #539 `b6a02e9d6` [RPC] Allow watchonly coins to be shown for listunspent (blondfrogs)
 - #543 `e4522ff07` [RPC] Segfault ozeety-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on eZEE spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting eZEE. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending eZEE. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/ozeety-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
