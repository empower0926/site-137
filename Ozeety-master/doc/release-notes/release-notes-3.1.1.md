OZEETY Core version *3.1.1* is now available from:  <https://github.com/ozeety-project/ozeety/releases>
...
OZEETY Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 eZEE created before block 1050020

The transition to v2 eZEE and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 eZEE to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending eZEE v1 will now be able to convert that into OZTG and then eZEE v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

eZEE stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by OZEETY stakers.
...
### Show progress percent for ezee reindex operations

When starting the wallet with `-reindexaccumulators` and/or `-reindexeshadow`, these operations can take a considerable time to complete depending on system hardware. A progress percent on the splash screen is now shown for these processes to avoid confusion in thinking that the wallet has frozen.
...
### Write to the eshadowDB in batches

Instead of using a separate write operation for each and every bit of data that needs to be flushed to disk, utilize leveldb's batch writing capability. The primary area of improvement this offers is when reindexing the eshadowDB (`-reindexeshadow`), which went from needing multiple hours on some systems to mere minutes.

Secondary improvement area is in ConnectBlock() when multiple eshadow transactions are involved.

### Resolution of excessive peer banning

It was found that following a forced closure of the OZEETY core wallet (ungraceful), a situation could arise that left partial/incomplete data in the disk cache. This caused the client to fail a basic sanity test and ban any peer which was sending the (complete) data. This, in turn, was causing the wallet to become stuck. This issue has been resolved client side by guarding against this partial/incomplete data in the disk cache.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove eZEE code from main.cpp (presstab)
 - #610 `6c3bc8c76` [Main] Check whether tx is in chain in ContextualCheckEshadowMint(). (presstab)
 - #624 `1a82aec96` [Core] Missing seesaw value for block 325000 (warrows)
 - #636 `d359c6136` [Main] Write to the eshadowDB in batches (Fuzzbawls)

### Build System
 - #605 `b4d82c944` [Build] Remove unnecessary BOOST dependency (Mrs-X)
 - #622 `b8c672c98` [Build] Make sure Boost headers are included for libeshadow (Fuzzbawls)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog eZEE elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for ezee reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 eZEE created before block 1050020. (presstab)
