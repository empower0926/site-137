OZEETY Core version *4.1.0* is now available from:  <https://github.com/ozeety-project/ozeety/releases>
...
Enabling major improvements over the keystore management, the OZEETY wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
An extended description of this large work can be found in the PR [here](https://github.com/OZEETY-Project/OZEETY/pull/1327).
...
Dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/OZEETY-Project/OZEETY/pull/1392)).
...
Addresses in the dropdown of the "Send Transaction" and "Send Delegation" widgets are now automatically sorted by label with ascending order ([#1393](https://github.com/OZEETY-Project/OZEETY/pull/1393)).

### Custom Fee

The custom fee selected when sending a transaction is now saved in the wallet database and persisted across multiple sends and wallet's restarts ([#1406](https://github.com/OZEETY-Project/OZEETY/pull/1406)). The fee is now also validated against the maximum value (10000 times `minRelayTxFee`) and minimum value (`minTxFee`) ([#1576](https://github.com/OZEETY-Project/OZEETY/pull/1576)).

### Include delegations in send

The send and cold-staking page present a checkbox to make the automatic input selection algorithm include delegated (P2CS) utxos if needed. The option is unchecked by default. ([#1391](https://github.com/OZEETY-Project/OZEETY/pull/1391))

### Optional Hiding of Staking Charts

The staking charts can now be hidden at startup (with a flag `--hidecharts`) or at runtime with a checkbox in settings --> options --> display ([#1475](https://github.com/OZEETY-Project/OZEETY/pull/1475)).
...
Restore the previous locking state ("locked" or "locked for staking only") when the action is completed. ([#1387](https://github.com/OZEETY-Project/OZEETY/pull/1387))

### External Change Address Warning

A new warning/confirmation dialog is displayed if a custom change address is not part of the wallet. ([#1527](https://github.com/OZEETY-Project/OZEETY/pull/1527))

### Cold Stake delegations marked in Coin Control

The Coin Control window now includes an icon next to the select checkbox when the UTXO is a Cold Stake delegation. This shares the space with the locked UTXO indicator icon, and locked UTXO's take priority in their icon display. ([#1470](https://github.com/OZEETY-Project/OZEETY/pull/1470))

### Hide eZEE balance info as needed

When the wallet contains no eZEE, the eZEE balance details will be hidden, reducing visual clutter.
...
```
"Confirmed","Date","Type","Label","Address","Amount (OZTG)","ID"
```
...
Automatic eZEE backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the ozeety.conf file or as startup flags):
- `autoezeebackup`
- `backupezee`
- `ezeebackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 OZTG, down  to 500 OZTG.
...
- The `-reservebalance` configuration/startup option has been removed ([#1373](https://github.com/OZEETY-Project/OZEETY/pull/1373)).


Dependencies
------------

The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/ozeety-project/ozeety/blob/master/depends/README.md) provides 5.9.7)
...
- "moneysupply" and "ezeeSupply" attributes in `getblock` output are removed.
- "isPublicSpend" boolean (optional) input parameter is removed from the following commands:
  - `createraweshadowspend`
  - `spendeshadow`
  - `spendeshadowmints`
  - `spendraweshadow`
...
- "mintchange" and "minimizechange" boolean input parameters are removed from the following commands:
  ...
  Mints are disabled, therefore it is no longer possible to mint the change of a eshadow spend. The change is minimized by default.
...
- The output of `getstakingstatus` was reworked. It now shows the following information:
  ```
  {
     ...
     "staking_enabled": true|false,      (boolean) whether staking is enabled/disabled in ozeety.conf
     "coldstaking_enabled": true|false,  (boolean) whether cold-staking is enabled/disabled in ozeety.conf
...
### GUI
 ...
 - #1306 `65ba128634` [GUI] Hide privacy widget when the wallet has no eZEE balance (furszy)
 - #1316 `bcb04a44a1` [Trivial] fix typo "recomended" in sendcustomfee dialog (random-zebra)
 - #1323 `e2c07184f9` [Qt] Hide eZEE balances when they are zero (Fuzzbawls)
...
### Wallet
 ...
 - #1382 `df2db0d5c6` [Wallet] Don't initialize ezeewallet on first run (Fuzzbawls)
...
### Core Systems
 ...
 - #1308 `ca912fc823` [eZEE] Public coin, a super for-each removed (furszy)
 ...
 - #1342 `dc530c072f` [Refactor] Move eshadow checks out of main (Fuzzbawls)
 - #1344 `e4041b1631` [Refactor] ChainParams::consensus - Part 2 (random-zebra)
 - #1359 `9869a384e2` [Chain][Bug] Fix mapEshadowSupply recalculation (random-zebra)
...
### Cleanup
 ...
 - #1276 `e5d6fdfd38` [Tests][Cleanup][Trivial] Remove legacy eshadow spends tests (random-zebra)
 ...
 - #1290 `2ceeb2cca0` [eZEE][Cleanup] Eshadow Cleanup 1: remove Accumulators values (random-zebra)
 - #1291 `16d7dac5f7` [eZEE][Cleanup] Eshadow Cleanup 2: remove CEZeeStake class (random-zebra)
 - #1293 `28e0048b3e` [eZEE][Cleanup] Eshadow Cleanup 3: remove old ZK proofs (random-zebra)
 - #1314 `185194bd7e` [eZEE][Cleanup] Eshadow Cleanup 4: further wallet cleaning (random-zebra)
 - #1322 `5bd387e18d` [eZEE][Cleanup] Eshadow Cleanup 5: further main.cpp cleaning (random-zebra)
 - #1326 `6bbd575860` [eZEE][Cleanup] Eshadow Cleanup 6: Remove eshadow mint checks (random-zebra)
