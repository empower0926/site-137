...
The build script we'll be using is contained in the OZEETY github repository ([contrib/gitian-build.py](https://github.com/ozeety-project/ozeety/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the OZEETY repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/OZEETY-Project/OZEETY/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, ozeety-detached-sigs, and ozeety GitHub repos.
