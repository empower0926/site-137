...
bool CTxIn::IsEshadowSpend() const
{
    return prevout.hash.IsNull() && scriptSig.IsEshadowSpend();
}

bool CTxIn::IsEshadowPublicSpend() const
{
    return scriptSig.IsEshadowPublicSpend();
}
...
std::string CTxIn::ToString() const
{
    ...
    if (prevout.IsNull())
        if(IsEshadowSpend())
            str += strprintf(", eshadowspend %s...", HexStr(scriptSig).substr(0, 25));
        else
...
bool CTxOut::IsEshadowMint() const
{
    return scriptPubKey.IsEshadowMint();
}
...
bool CTransaction::HasEshadowSpendInputs() const
{
    for (const CTxIn& txin: vin) {
        if (txin.IsEshadowSpend() || txin.IsEshadowPublicSpend())
            return true;
    }
    return false;
}

bool CTransaction::HasEshadowMintOutputs() const
{
...
bool CTransaction::HasEshadowPublicSpendInputs() const
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between oztg and ezee
    for(const CTxIn& txin : vin) {
...
bool CTransaction::IsCoinStake() const
{
    ...
    bool fAllowNull = vin[0].IsEshadowSpend();
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // OZEETY: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
...
CAmount CTransaction::GetEshadowSpent() const
{
    CAmount nValueOut = 0;
    for (const CTxIn& txin : vin) {
