...
bool COztgStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsEshadowSpend())
        return error("%s: unable to initialize COztgStake from eshadow spend", __func__);
...
bool COztgStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool COztgStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool COztgStake::GetTxOutFrom(CTxOut& out) const
{
...
bool COztgStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount COztgStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool COztgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream COztgStake::GetUniqueness() const
{
...
CBlockIndex* COztgStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool COztgStake::ContextCheck(int nHeight, uint32_t nTime)
{
