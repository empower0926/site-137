 ...
 * @file       Coin.cpp
 *
 * @brief      PublicCoin and PrivateCoin classes for the Eshadow library.
 *
...
//PublicCoin class
PublicCoin::PublicCoin(const EshadowParams* p):
    params(p) {
...
PublicCoin::PublicCoin(const EshadowParams* p, const CBigNum& coin, const CoinDenomination d):
...
//PrivateCoin class
PrivateCoin::PrivateCoin(const EshadowParams* p, const CoinDenomination denomination, bool fMintNew): params(p), publicCoin(p) {
...
PrivateCoin::PrivateCoin(const EshadowParams* p, const CoinDenomination denomination, const CBigNum& bnSerial,
...
bool GenerateKeyPair(const CBigNum& bnGroupOrder, const uint256& nPrivkey, CKey& key, CBigNum& bnSerial)
{
    ...
    // This builds off of Tim Ruffing's work on libeshadow, but has a different implementation
    CKey keyPair;
    if (nPrivkey.IsNull())
    ...
    hashPubKey >>= libeshadow::PrivateCoin::V2_BITSHIFT;
...
void PrivateCoin::mintCoin(const CoinDenomination denomination) {
    ...
    for(uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
        ...
        if (coin.getCommitmentValue().isPrime(ESHADOW_MINT_PRIME_PARAM) &&
    ...
    throw std::runtime_error("Unable to mint a new Eshadow (too many attempts)");
}
...
void PrivateCoin::mintCoinFast(const CoinDenomination denomination) {
    ...
    for (uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
        ...
        // in the appropriate range. If not, we'll throw this coin
        // away and generate a new one.
        if (commitmentValue.isPrime(ESHADOW_MINT_PRIME_PARAM) &&
...
bool IsValidSerial(const EshadowParams* params, const CBigNum& bnSerial)
{
    if (bnSerial <= 0)
        return false;
...
bool IsValidCommitmentToCoinRange(const EshadowParams* params, const CBigNum& bnCommitment)
{
