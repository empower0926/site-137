...
public:
    ...
    PublicCoin(const EshadowParams* p, Stream& strm) : params(p)
    {
        strm >> *this;
    }

    PublicCoin(const EshadowParams* p);
     ...
     */
    PublicCoin(const EshadowParams* p, const CBigNum& coin, const CoinDenomination d);
...
public:
    ...
    template <typename Stream>
    PrivateCoin(const EshadowParams* p, Stream& strm) : params(p), publicCoin(p)
    {
        strm >> *this;
    }
    PrivateCoin(const EshadowParams* p, const CoinDenomination denomination, bool fMintNew = true);
    PrivateCoin(const EshadowParams* p, const CoinDenomination denomination, const CBigNum& bnSerial, const CBigNum& bnRandomness);
...
private:
     ...
     * @throws EshadowException if the process takes too long
     *
     * Generates a new Eshadow by (a) selecting a random serial
