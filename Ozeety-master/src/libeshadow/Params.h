...
* @file       Params.h
*
* @brief      Parameter classes for Eshadow.
*
...
public:
	/** @brief Construct a set of Eshadow parameters from a modulus "N".
	* @param N                A trusted RSA modulus
	* @param securityLevel    A security level expressed in symmetric bits (default 80)
	*
	* Allocates and derives a set of Eshadow parameters from
	...
	* be known to any party, or the security of Eshadow is
...
public:
	...
	* in length. 3072 bits is strongly recommended.
	**/
	EshadowParams(CBigNum accumulatorModulus,
	       uint32_t securityLevel = ESHADOW_DEFAULT_SECURITYLEVEL);
