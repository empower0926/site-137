...
CoinDenomination IntToEshadowDenomination(int64_t amount)
{
...
int64_t EshadowDenominationToInt(const CoinDenomination& denomination)
{
    int64_t Value = 0;
    switch (denomination) {
...
// use case: converting OZTG to eZEE without user worrying about denomination math themselves
CoinDenomination AmountToClosestDenomination(CAmount nAmount, CAmount& nRemaining)
{
    ...
    for (unsigned int i = 0; i < eshadowDenomList.size(); i++) {
        denomination = eshadowDenomList[i];
        ...
        if (denomination > nConvert && i) {
            CoinDenomination d = eshadowDenomList[i - 1];
...
CAmount EshadowDenominationToAmount(const CoinDenomination& denomination)
{
    CAmount nValue = COIN * EshadowDenominationToInt(denomination);
    return nValue;
}
...
CoinDenomination get_denomination(std::string denomAmount) {
    ...
    return IntToEshadowDenomination(val);
}
...
int64_t get_amount(std::string denomAmount) {
    ...
    if (denom == ZQ_ERROR) {
        ...
        nAmount = EshadowDenominationToAmount(denom);
    }
    return nAmount;
}
