 ...
 * @file       CoinSpend.h
 *
 * @brief      CoinSpend class for the Eshadow library.
 *
...
{
// Lagacy eZEE - Only for serialization
...
// Lagacy eZEE - Only for serialization
/** The complete proof needed to spend a eshadow.
...
public:
    ...
    int getCoinVersion() const { return libeshadow::ExtractVersionFromSerial(coinSerialNumber); }
    ...
    bool HasValidSerial(EshadowParams* params) const;
    bool HasValidSignature() const;
    void setTxOutHash(uint256 txOutHash) { this->ptxHash = txOutHash; };
    void setDenom(libeshadow::CoinDenomination denom) { this->denomination = denom; }
    void setPubKey(CPubKey pkey, bool fUpdateSerial = false);

    CBigNum CalculateValidSerial(EshadowParams* params);
