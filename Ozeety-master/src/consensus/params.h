...
struct Params {
    ...
    // validation by-pass
    int64_t nOzeetyBadBlockTime;
    unsigned int nOzeetyBadBlockBits;
    ...
    /*
     * (Legacy) Eshadow consensus params
     */
    std::string ZC_Modulus;  // parsed in Eshadow_Params (either as hex or dec string)
    ...
    libeshadow::EshadowParams* Eshadow_Params(bool useModulusV1) const
    {
        static CBigNum bnHexModulus = 0;
        if (!bnHexModulus) bnHexModulus.SetHex(ZC_Modulus);
        static libeshadow::EshadowParams ZCParamsHex = libeshadow::EshadowParams(bnHexModulus);
        static CBigNum bnDecModulus = 0;
        if (!bnDecModulus) bnDecModulus.SetDec(ZC_Modulus);
        static libeshadow::EshadowParams ZCParamsDec = libeshadow::EshadowParams(bnDecModulus);
