...
bool CheckTransaction(const CTransaction& tx, bool fEshadowActive, bool fRejectBadUTXO, CValidationState& state, bool fFakeSerialAttack, bool fColdStakingActive)
{
    ...
    // Size limits
    unsigned int nMaxSize = MAX_ESHADOW_TX_SIZE;
    ...
    std::set<CBigNum> vEshadowSpendSerials;
    int nZCSpendCount = 0;
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        //duplicate zcspend serials are checked in CheckEshadowSpend()
        if (!txin.IsEshadowSpend()) {
            vInOutPoints.insert(txin.prevout);
        } else if (!txin.IsEshadowPublicSpend()) {
            nZCSpendCount++;
        }
    }
    ...
    if (fEshadowActive) {
        if (nZCSpendCount > consensus.ZC_MaxSpendsPerTx)
            return state.DoS(100, error("CheckTransaction() : there are more eshadow spends than are allowed in one transaction"));

        //require that a eshadowspend only has inputs that are eshadows
        if (tx.HasEshadowSpendInputs()) {
            for (const CTxIn& in : tx.vin) {
                if (!in.IsEshadowSpend() && !in.IsEshadowPublicSpend())
                    return state.DoS(100,
                                     error("CheckTransaction() : eshadowspend contains inputs that are not eshadows"));
            }
            ...
            if (!CheckEshadowSpend(tx, fVerifySignature, state, fFakeSerialAttack))
                return state.DoS(100, error("CheckTransaction() : invalid eshadow spend"));
        }
    }
    ...
    if (tx.IsCoinBase()) {
    ...
    } else if (fEshadowActive && tx.HasEshadowSpendInputs()) {
        if (tx.vin.size() < 1)
            return state.DoS(10, false, REJECT_INVALID, "bad-zc-spend-min-inputs");
        if (tx.HasEshadowPublicSpendInputs()) {
            // tx has public eshadow spend inputs
            ...
            // tx has regular eshadow spend inputs
        ...
        for (const CTxIn& txin : tx.vin)
            if (txin.prevout.IsNull() && (fEshadowActive && !txin.IsEshadowSpend()))
