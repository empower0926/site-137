...
// Copyright (c) 2016-2020 The OZEETY developers
...
CEshadowDB::CEshadowDB(size_t nCacheSize, bool fMemory, bool fWipe) : CDBWrapper(GetDataDir() / "eshadow", nCacheSize, fMemory, fWipe)
{
}

bool CEshadowDB::WriteCoinMintBatch(const std::vector<std::pair<libeshadow::PublicCoin, uint256> >& mintInfo)
{
    CDBBatch batch;
    size_t count = 0;
    for (std::vector<std::pair<libeshadow::PublicCoin, uint256> >::const_iterator it=mintInfo.begin(); it != mintInfo.end(); it++) {
        libeshadow::PublicCoin pubCoin = it->first;
...
bool CEshadowDB::ReadCoinMint(const CBigNum& bnPubcoin, uint256& hashTx)
{
    return ReadCoinMint(GetPubCoinHash(bnPubcoin), hashTx);
}

bool CEshadowDB::ReadCoinMint(const uint256& hashPubcoin, uint256& hashTx)
{
    return Read(std::make_pair('m', hashPubcoin), hashTx);
}

bool CEshadowDB::EraseCoinMint(const CBigNum& bnPubcoin)
{
    uint256 hash = GetPubCoinHash(bnPubcoin);
    return Erase(std::make_pair('m', hash));
}

bool CEshadowDB::WriteCoinSpendBatch(const std::vector<std::pair<libeshadow::CoinSpend, uint256> >& spendInfo)
{
    CDBBatch batch;
    size_t count = 0;
    for (std::vector<std::pair<libeshadow::CoinSpend, uint256> >::const_iterator it=spendInfo.begin(); it != spendInfo.end(); it++) {
...
bool CEshadowDB::ReadCoinSpend(const CBigNum& bnSerial, uint256& txHash)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << bnSerial;
    uint256 hash = Hash(ss.begin(), ss.end());
...
bool CEshadowDB::ReadCoinSpend(const uint256& hashSerial, uint256 &txHash)
{
    return Read(std::make_pair('s', hashSerial), txHash);
}

bool CEshadowDB::EraseCoinSpend(const CBigNum& bnSerial)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << bnSerial;
    uint256 hash = Hash(ss.begin(), ss.end());
...
bool CEshadowDB::WipeCoins(std::string strType)
{
...
// Legacy Eshadow Database
static const char LZC_ACCUMCS = 'A';
static const char LZC_MAPSUPPLY = 'M';

bool CEshadowDB::WriteZCSupply(const std::map<libeshadow::CoinDenomination, int64_t>& mapZCS)
{
    return Write(LZC_MAPSUPPLY, mapZCS);
}

bool CEshadowDB::ReadZCSupply(std::map<libeshadow::CoinDenomination, int64_t>& mapZCS) const
{
    return Read(LZC_MAPSUPPLY, mapZCS);
}

bool CEshadowDB::WriteAccChecksum(const uint32_t& nChecksum, const libeshadow::CoinDenomination denom, const int nHeight)
{
    return Write(std::make_pair(LZC_ACCUMCS, std::make_pair(nChecksum, denom)), nHeight);
}

bool CEshadowDB::ReadAccChecksum(const uint32_t& nChecksum, const libeshadow::CoinDenomination denom, int& nHeightRet)
{
    return Read(std::make_pair(LZC_ACCUMCS, std::make_pair(nChecksum, denom)), nHeightRet);
}

bool CEshadowDB::EraseAccChecksum(const uint32_t& nChecksum, const libeshadow::CoinDenomination denom)
{
    return Erase(std::make_pair(LZC_ACCUMCS, std::make_pair(nChecksum, denom)));
}

bool CEshadowDB::WipeAccChecksums()
{
    boost::scoped_ptr<CDBIterator> pcursor(NewIterator());
