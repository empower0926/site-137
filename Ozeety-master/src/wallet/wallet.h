...
#include "ezee/ezeewallet.h"
#include "ezee/ezeetracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_OZEETY = 61000, // inherited version..
...
// Possible states for eZEE send
enum EshadowSpendStatus {
    EZEE_SPEND_OKAY = 0,                            // No error
    EZEE_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    EZEE_WALLET_LOCKED = 2,                         // Wallet was locked
    EZEE_COMMIT_FAILED = 3,                         // Commit failed, reset status
    EZEE_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    EZEE_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    EZEE_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    EZEE_TRX_CREATE = 7,                            // Everything related to create the transaction
    EZEE_TRX_CHANGE = 8,                            // Everything related to transaction change
    EZEE_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    EZEE_INVALID_COIN = 10,                         // Selected mint coin is not valid
    EZEE_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    EZEE_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    EZEE_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    EZEE_SPENT_USED_EZEE = 14,                      // Coin has already been spend
    EZEE_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    EZEE_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 OZTG output and keys which can be used for the Masternode
    ...
    /* Legacy ZC - implementations in wallet_eshadow.cpp */
    ...
    void doEZeeRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckEZEE);

    //- ZC Mints (Only for regtest)
    std::string MintEshadow(CAmount nValue, CWalletTx& wtxNew, std::vector<CDeterministicMint>& vDMints, const CCoinControl* coinControl = NULL);
    std::string MintEshadowFromOutPoint(CAmount nValue, CWalletTx& wtxNew, std::vector<CDeterministicMint>& vDMints, const std::vector<COutPoint> vOutpts);
    bool CreateEZEEOutPut(libeshadow::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool CreateEshadowMintTransaction(const CAmount nValue,
    ...
    // - ZC PublicSpends
    bool SpendEshadow(CAmount nAmount, CWalletTx& wtxNew, CEshadowSpendReceipt& receipt, std::vector<CEshadowMint>& vMintsSelected, std::list<std::pair<CTxDestination,CAmount>> addressesTo, CTxDestination* changeAddress = nullptr);
    bool MintsToInputVectorPublicSpend(std::map<CBigNum, CEshadowMint>& mapMintsSelected, const uint256& hashTxOut, std::vector<CTxIn>& vin, CEshadowSpendReceipt& receipt, libeshadow::SpendType spendType, CBlockIndex* pindexCheckpoint = nullptr);
    ...
    // - ZC Balances
    CAmount GetEshadowBalance(bool fMatureOnly) const;
    CAmount GetUnconfirmedEshadowBalance() const;
    CAmount GetImmatureEshadowBalance() const;
    std::map<libeshadow::CoinDenomination, CAmount> GetMyEshadowDistribution() const;

    // eZEE wallet
    CeZEEWallet* zwalletMain{nullptr};
    std::unique_ptr<CeZEETracker> ezeeTracker{nullptr};
    void setZWallet(CeZEEWallet* zwallet);
    CeZEEWallet* getZWallet();
    bool IsMyEshadowSpend(const CBigNum& bnSerial) const;
    bool IsMyMint(const CBigNum& bnValue) const;
    std::string ResetMintEshadow();
    std::string ResetSpentEshadow();
    void ReconsiderEshadows(std::list<CEshadowMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored);
    bool GetEshadowKey(const CBigNum& bnSerial, CKey& key);
    bool GetMint(const uint256& hashSerial, CEshadowMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CEshadowMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libeshadow::CoinDenomination& denom);
    // Eshadow entry changed. (called with lock cs_wallet held)
    boost::signals2::signal<void(CWallet* wallet, const std::string& pubCoin, const std::string& isUsed, ChangeType status)> NotifyEshadowChanged;
    // eZEE reset
    boost::signals2::signal<void()> NotifyeZEEReset;
