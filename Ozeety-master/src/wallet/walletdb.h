...
#include "libeshadow/Accumulator.h"
...
public:
    ...
    bool WriteEshadowMint(const CEshadowMint& eshadowMint);
    bool EraseEshadowMint(const CEshadowMint& eshadowMint);
    bool ReadEshadowMint(const CBigNum &bnPubcoinValue, CEshadowMint& eshadowMint);
    bool ReadEshadowMint(const uint256& hashPubcoin, CEshadowMint& mint);
    bool ArchiveMintOrphan(const CEshadowMint& eshadowMint);
    bool ArchiveDeterministicOrphan(const CDeterministicMint& dMint);
    bool UnarchiveEshadowMint(const uint256& hashPubcoin, CEshadowMint& mint);
    bool UnarchiveDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint);
    std::list<CEshadowMint> ListMintedCoins();
    std::list<CDeterministicMint> ListDeterministicMints();
    std::list<CEshadowSpend> ListSpentCoins();
    std::list<CBigNum> ListSpentCoinsSerial();
    std::list<CEshadowMint> ListArchivedEshadows();
    std::list<CDeterministicMint> ListArchivedDeterministicMints();
    bool WriteEshadowSpendSerialEntry(const CEshadowSpend& eshadowSpend);
    bool EraseEshadowSpendSerialEntry(const CBigNum& serialEntry);
    bool ReadEshadowSpendSerialEntry(const CBigNum& bnSerial);
    bool WriteCurrentSeedHash(const uint256& hashSeed);
    bool ReadCurrentSeedHash(uint256& hashSeed);
    bool WriteEZEESeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadEZEESeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadEZEESeed_deprecated(uint256& seed);
    bool EraseEZEESeed();
    bool EraseEZEESeed_deprecated();

    bool WriteEZEECount(const uint32_t& nCount);
    bool ReadEZEECount(uint32_t& nCount);
