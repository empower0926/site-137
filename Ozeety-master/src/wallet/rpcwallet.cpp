...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "  \"account\" : \"account\"                 (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start ozeetyd with -deprecatedrpc=accounts. The account associated with the address, \"\" is the default account\n"
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new OZEETY address for receiving payments.\n"
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from OZEETY address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get OZEETY address from addressBook");
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccountaddress (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccountaddress is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"ozeetyaddress\"   (string) The account ozeety address.\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new OZEETY address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "setaccount") {
        if (request.fHelp) {
            throw std::runtime_error("setaccount (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "setaccount is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"ozeetyaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"   (string, required) The ozeety address to be associated with a label.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccount (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccount is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "getaccount \"ozeetyaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"  (string, required) The ozeety address for account lookup.\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaddressesbyaccount (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaddressesbyaccount is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "[                     (json array of string)\n"
            "  \"ozeetyaddress\"  (string) a ozeety address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"ozeetyaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"  (string, required) The ozeety address to send to.\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OZEETY staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OZEETY spending address");
...
UniValue sendtoaddressix(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"ozeetyaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"ozeetyaddress\",     (string) The ozeety address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"ozeetyaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"  (string, required) The ozeety address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"ozeetyaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given ozeetyaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"  (string, required) The ozeety address for transactions.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "getreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("getreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() > 4 && IsDeprecatedRPCEnabled("accounts")) || (request.params.size() != 0 && !IsDeprecatedRPCEnabled("accounts")))
        throw std::runtime_error(
            ...
            "\nIf account is not specified, returns the server's total available balance (excluding eshadows).\n"
            ...
            "                    To use this deprecated argument, start ozeetyd with -deprecatedrpc=accounts."
            "2. minconf          (numeric, optional, default=1) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start ozeetyd with -deprecatedrpc=accounts. Only include transactions confirmed at least this many times.\n"
            "3. includeWatchonly (bool, optional, default=false) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start ozeetyd with -deprecatedrpc=accounts. Also include balance in watchonly addresses (see 'importaddress')\n"
            "4. includeDelegated (bool, optional, default=true) Only available when specifying an account.\n"
            "                    To use this argument, start ozeetyd with -deprecatedrpc=accounts. Also include balance delegated to cold stakers\n"
...
UniValue movecmd(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("move (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "move is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("sendfrom (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "sendfrom is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toozeetyaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nDEPRECATED (use sendtoaddress). Send an amount from an account to a ozeety address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toozeetyaddress\"  (string, required) The ozeety address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "sendmany \"\" {\"address\":amount,...} ( minconf \"comment\" includeDelegated )\n"
            ...
            "ozeetyd with -deprecatedrpc=accounts\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The ozeety address is the key, the numeric amount in OZTG is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a OZEETY address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of ozeety addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) ozeety address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"      (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"ozeetyaddress\"  (string) A ozeety address associated with the keys.\n"
...
UniValue listreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "listreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("listreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "listtransactions ( \"dummy\" count from includeWatchonly includeDelegated )\n"
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"ozeetyaddress\",    (string) The ozeety address of the transaction. Not present for \n"
...
UniValue listaccounts(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("listaccounts (Deprecated, will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listaccounts is deprecated and will be removed in v5.0. To use this command, start ozeetyd with -deprecatedrpc=accounts.");
    }
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"account\":\"accountname\",       (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start ozeetyd with -deprecatedrpc=accounts. The account name associated with the transaction. Will be \"\" for the default account.\n"
            "    \"address\":\"ozeetyaddress\",    (string) The ozeety address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"account\" : \"accountname\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start ozeetyd with -deprecatedrpc=accounts. The account name involved in the transaction, can be \"\" for the default account.\n"
            "      \"address\" : \"ozeetyaddress\",   (string) The ozeety address involved in the transaction\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            HelpExampleCli("signmessage", "\"ozeetyaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; ozeety server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "3. \"addresses\"    (string) A json array of ozeety addresses to filter\n"
                "    [\n"
                ...
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",  (string) the ozeety address\n"
                "    \"label\" : \"label\",      (string) The associated label, or \"\" for the default label\n"
                "    \"account\" : \"account\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start ozeetyd with -deprecatedrpc=accounts. Backwards compatible alias for label.\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid OZEETY address: ") + input.get_str());
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total OZTG balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the OZTG balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the OZTG balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in OZTG\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in OZTG\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in OZTG\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same OZEETY address\n"
...
UniValue geteshadowbalance(const JSONRPCRequest& request)
{

    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "geteshadowbalance\n"
            "\nReturn the wallet's total eZEE balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total eZEE balance.\n"

            "\nExamples:\n" +
            HelpExampleCli("geteshadowbalance", "") + HelpExampleRpc("geteshadowbalance", ""));
    ...
    EnsureWalletIsUnlocked(true);
        ...
        UniValue ret(UniValue::VOBJ);
        ret.push_back(Pair("Total", ValueFromAmount(pwalletMain->GetEshadowBalance(false))));
        ret.push_back(Pair("Mature", ValueFromAmount(pwalletMain->GetEshadowBalance(true))));
        ret.push_back(Pair("Unconfirmed", ValueFromAmount(pwalletMain->GetUnconfirmedEshadowBalance())));
        ret.push_back(Pair("Immature", ValueFromAmount(pwalletMain->GetImmatureEshadowBalance())));
        return ret;

}

UniValue listmintedeshadows(const JSONRPCRequest& request)
{

    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            "listmintedeshadows (fVerbose) (fMatureOnly)\n"
            "\nList all eZEE mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"version\": n,   (numeric) Eshadow version number.\n"
            "    \"eZEE ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("listmintedeshadows", "") + HelpExampleRpc("listmintedeshadows", "") +
            HelpExampleCli("listmintedeshadows", "true") + HelpExampleRpc("listmintedeshadows", "true") +
            HelpExampleCli("listmintedeshadows", "true true") + HelpExampleRpc("listmintedeshadows", "true, true"));
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->ezeeTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("version", m.nVersion));                 // Eshadow version
            objMint.push_back(Pair("eZEE ID", m.hashPubcoin.GetHex()));     // PubCoin
            int denom = libeshadow::EshadowDenominationToInt(m.denom);
            ...
            if (m.hashStake.IsNull()) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->ezeeTracker->UpdateState(m);
                }
            }
...
UniValue listeshadowamounts(const JSONRPCRequest& request)
{

    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "listeshadowamounts\n"
            "\nGet information about your eshadow amounts.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("listeshadowamounts", "") + HelpExampleRpc("listeshadowamounts", ""));
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->ezeeTracker->ListMints(true, true, true);
    ...
    UniValue ret(UniValue::VARR);
    for (const auto& m : libeshadow::eshadowDenomList) {
        UniValue val(UniValue::VOBJ);
        val.push_back(Pair("denomination", libeshadow::EshadowDenominationToInt(m)));
...
UniValue listspenteshadows(const JSONRPCRequest& request)
{

    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "listspenteshadows\n"
            "\nList all the spent eZEE mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("listspenteshadows", "") + HelpExampleRpc("listspenteshadows", ""));
...
UniValue minteshadow(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "minteshadow amount ( utxos )\n"
            "\nMint the specified eZEE amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Oztg to convert to eZEE\n"
            ...
            HelpExampleCli("minteshadow", "50") +
            "\nMint 13 from a specific output\n" +
            HelpExampleCli("minteshadow", "13 \"[{\\\"txid\\\":\\\"a08e6907dbbd3d809776dbfc5d82e371b764ed838b5655e72f463568df1aadf0\\\",\\\"vout\\\":1}]\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("minteshadow", "13, \"[{\\\"txid\\\":\\\"a08e6907dbbd3d809776dbfc5d82e371b764ed838b5655e72f463568df1aadf0\\\",\\\"vout\\\":1}]\""));


    if (!Params().IsRegTestNet())
        throw JSONRPCError(RPC_WALLET_ERROR, "eZEE minting is DISABLED");
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ESHADOW_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "eZEE is currently disabled due to maintenance.");
    ...
    if (request.params.size() == 2)
    {
        ...
        strError = pwalletMain->MintEshadowFromOutPoint(nAmount, wtx, vDMints, vOutpts);
    } else
    {
        strError = pwalletMain->MintEshadow(nAmount, wtx, vDMints);
    }
    ...
    for (CDeterministicMint dMint : vDMints) {
        ...
        m.push_back(Pair("denomination", ValueFromAmount(libeshadow::EshadowDenominationToAmount(dMint.GetDenomination()))));
...
UniValue spendeshadow(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2 || request.params.size() < 1)
        throw std::runtime_error(
            "spendeshadow amount ( \"address\" )\n"
            "\nSpend eZEE to a OZTG address.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in OZTG.\n"
            "      \"address\": \"xxx\"         (string) OZTG address or \"eshadowmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("spendeshadow", "5000 \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"") +
            HelpExampleRpc("spendeshadow", "5000 \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\""));
    ...
    std::vector<CEshadowMint> vMintsSelected;
    return DoEzeeSpend(nAmount, vMintsSelected, address_str);
}


UniValue spendeshadowmints(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "spendeshadowmints mints_list ( \"address\" ) \n"
            "\nSpend eZEE mints to a OZTG address.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. mints_list     (string, required) A json array of eshadow mints serial hashes\n"
            "2. \"address\"     (string, optional, default=change) Send to specified address or to a new change address.\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("spendeshadowmints", "'[\"0d8c16eee7737e3cc1e4e70dc006634182b175e039700931283b202715a0818f\", \"dfe585659e265e6a509d93effb906d3d2a0ac2fe3464b2c3b6d71a3ef34c8ad7\"]' \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\"") +
            HelpExampleRpc("spendeshadowmints", "[\"0d8c16eee7737e3cc1e4e70dc006634182b175e039700931283b202715a0818f\", \"dfe585659e265e6a509d93effb906d3d2a0ac2fe3464b2c3b6d71a3ef34c8ad7\"], \"DMJRSsuU9zfyrvxVaAEFQqK4MxZg6vgeS6\""));
    ...
    if (arrMints.size() == 0)
        throw JSONRPCError(RPC_WALLET_ERROR, "No eshadow selected");
...
extern UniValue DoEzeeSpend(const CAmount nAmount, std::vector<CEshadowMint>& vMintsSelected, std::string address_str)
{
    ...
    EnsureWalletIsUnlocked();
    fSuccess = pwalletMain->SpendEshadow(nAmount, wtx, receipt, vMintsSelected, outputs, nullptr);
    ...
    for (CEshadowSpend spend : receipt.GetSpends()) {
        ...
        nValueIn += libeshadow::EshadowDenominationToAmount(spend.GetDenomination());
    }
    ...
    for (unsigned int i = 0; i < wtx.vout.size(); i++) {
        ...
        if(txout.IsEshadowMint())
            out.push_back(Pair("address", "eshadowmint"));
...
UniValue resetminteshadow(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "resetminteshadow ( fullscan )\n"
            "\nScan the blockchain for all of the eshadows that are held in the wallet.dat.\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("resetminteshadow", "true") + HelpExampleRpc("resetminteshadow", "true"));

    LOCK2(cs_main, pwalletMain->cs_wallet);

    CWalletDB walletdb(pwalletMain->strWalletFile);
    CeZEETracker* ezeeTracker = pwalletMain->ezeeTracker.get();
...
UniValue resetspenteshadow(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "resetspenteshadow\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("resetspenteshadow", "") + HelpExampleRpc("resetspenteshadow", ""));

    LOCK2(cs_main, pwalletMain->cs_wallet);

    CWalletDB walletdb(pwalletMain->strWalletFile);
    CeZEETracker* ezeeTracker = pwalletMain->ezeeTracker.get();
    std::set<CMintMeta> setMints = ezeeTracker->ListMints(false, false, false);
...
UniValue getarchivedeshadow(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getarchivedeshadow\n"
            "\nDisplay eshadows that were archived because they were believed to be orphans.\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("getarchivedeshadow", "") + HelpExampleRpc("getarchivedeshadow", ""));
    ...
    for (const CEshadowMint& mint : listMints) {
        UniValue objMint(UniValue::VOBJ);
    ...
    for (const CDeterministicMint& dMint : listDMints) {
        ...
        objDMint.push_back(Pair("denomination", ValueFromAmount(libeshadow::EshadowDenominationToAmount(dMint.GetDenomination()))));
...
UniValue exporteshadows(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            "exporteshadows include_spent ( denomination )\n"
            "\nExports eshadow mints that are held by this wallet.dat\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"include_spent\"        (bool, required) Include mints that have already been spent\n"
            "2. \"denomination\"         (integer, optional) Export a specific denomination of eZEE\n"

            "\nResult:\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's eZEE serial hash \n"
            "    \"d\": n,         (numeric) the mint's eshadow denomination \n"
            ...
            "    \"v\": version,   (numeric) The version of the eZEE\n"
            "    \"k\": \"privkey\"  (string) The eZEE private key (V2+ eZEE only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"

            "\nExamples:\n" +
            HelpExampleCli("exporteshadows", "false 5") + HelpExampleRpc("exporteshadows", "false 5"));
    ...
    libeshadow::CoinDenomination denomination = libeshadow::ZQ_ERROR;
    if (request.params.size() == 2)
        denomination = libeshadow::IntToEshadowDenomination(request.params[1].get_int());

    CeZEETracker* ezeeTracker = pwalletMain->ezeeTracker.get();
    std::set<CMintMeta> setMints = ezeeTracker->ListMints(!fIncludeSpent, false, false);

    UniValue jsonList(UniValue::VARR);
    for (const CMintMeta& meta : setMints) {
        if (denomination != libeshadow::ZQ_ERROR && denomination != meta.denom)
            continue;
...
UniValue importeshadows(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() == 0)
        throw std::runtime_error(
            "importeshadows importdata \n"
            "\n[{\"d\":denomination,\"p\":\"pubcoin_hex\",\"s\":\"serial_hex\",\"r\":\"randomness_hex\",\"t\":\"txid\",\"h\":height, \"u\":used},{\"d\":...}]\n"
            "\nImport eshadow mints.\n"
            "Adds raw eshadow mints to the wallet.dat\n"
            "Note it is recommended to use the json export created from the exporteshadows RPC call\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"importdata\"    (string, required) A json array of json objects containing eshadow mints\n"

            "\nResult:\n"
            "{\n"
            "  \"added\": n,        (numeric) The quantity of eshadow mints that were added\n"
            "  \"value\": amount    (numeric) The total eZEE value of eshadow mints that were added\n"
            "}\n"

            "\nExamples\n" +
            HelpExampleCli("importeshadows", "\'[{\"d\":100,\"p\":\"mypubcoin\",\"s\":\"myserial\",\"r\":\"randomness_hex\",\"t\":\"mytxid\",\"h\":104923, \"u\":false},{\"d\":5,...}]\'") +
            HelpExampleRpc("importeshadows", "[{\"d\":100,\"p\":\"mypubcoin\",\"s\":\"myserial\",\"r\":\"randomness_hex\",\"t\":\"mytxid\",\"h\":104923, \"u\":false},{\"d\":5,...}]"));
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        libeshadow::CoinDenomination denom = libeshadow::IntToEshadowDenomination(d);
        CBigNum bnValue = 0;
        ...
        CPrivKey privkey;
        if (nVersion >= libeshadow::PrivateCoin::PUBKEY_VERSION) {
        ...
        CEshadowMint mint(denom, bnValue, bnRandom, bnSerial, fUsed, nVersion, &privkey);
        mint.SetTxHash(txid);
        mint.SetHeight(nHeight);
        pwalletMain->ezeeTracker->Add(mint, true);
        count++;
        nValue += libeshadow::EshadowDenominationToAmount(denom);
    }
...
UniValue reconsidereshadows(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "reconsidereshadows\n"
            "\nCheck archived eZEE list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"txid\" : \"xxx\",           (string) the mint's eshadow denomination \n"
            "    \"denomination\" : amount,  (numeric) the mint's eshadow denomination\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("reconsidereshadows", "") + HelpExampleRpc("reconsidereshadows", ""));
    ...
    pwalletMain->ReconsiderEshadows(listMints, listDMints);
    ...
    for (const CDeterministicMint& dMint : listDMints) {
        ...
        objMint.push_back(Pair("denomination", FormatMoney(libeshadow::EshadowDenominationToAmount(dMint.GetDenomination()))));
...
UniValue setezeeseed(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setezeeseed \"seed\"\n"
            "\nSet the wallet's deterministic ezee seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic ezee seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setezeeseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setezeeseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CeZEEWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getezeeseed(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "getezeeseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic eZEE seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getezeeseed", "") + HelpExampleRpc("getezeeseed", ""));
...
UniValue generatemintlist(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic eZEE seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential eZEE to start with.\n"
            "2. \"range\"  : n,  (numeric) How many eZEE to generate.\n"
    ...
    CeZEEWallet* zwallet = pwalletMain->zwalletMain;

    UniValue arrRet(UniValue::VARR);
    for (int i = nCount; i < nCount + nRange; i++) {
        libeshadow::CoinDenomination denom = libeshadow::CoinDenomination::ZQ_ONE;
        libeshadow::PrivateCoin coin(Params().GetConsensus().Eshadow_Params(false), denom, false);
...
UniValue dezeestate(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
                "dezeestate\n"
                        "\nThe current state of the mintpool of the deterministic eZEE wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dezee_count", nCount));
...
void static SearchThread(CeZEEWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToEZEE(eshadowSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdezee(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "searchdezee\n"
            "\nMake an extended search for deterministically generated eZEE that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential eZEE to start with.\n"
            "2. \"range\"       (numeric) How many eZEE to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdezee", "1, 100, 2") + HelpExampleRpc("searchdezee", "1, 100, 2"));
    ...
    boost::thread_group* dezeeThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dezeeThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dezeeThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->ezeeTracker->GetSerialHashes());
...
UniValue spendraweshadow(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 6)
        throw std::runtime_error(
            "spendraweshadow \"serialHex\" denom \"randomnessHex\" \"priv key\" ( \"address\" \"mintTxId\" )\n"
            "\nCreate and broadcast a TX spending the provided zericoin.\n"

            "\nArguments:\n"
            "1. \"serialHex\"        (string, required) A eshadow serial number (hex)\n"
            "2. \"randomnessHex\"    (string, required) A eshadow randomness value (hex)\n"
            "3. denom                (numeric, required) A eshadow denomination (decimal)\n"
            "4. \"priv key\"         (string, required) The private key associated with this coin (hex)\n"
            "5. \"address\"          (string, optional) OZEETY address to spend to. If not specified, "
            ...
            "\nExamples\n" +
            HelpExampleCli("spendraweshadow", "\"f80892e78c30a393ef4ab4d5a9d5a2989de6ebc7b976b241948c7f489ad716a2\" \"a4fd4d7248e6a51f1d877ddd2a4965996154acc6b8de5aa6c83d4775b283b600\" 100 \"xxx\"") +
            HelpExampleRpc("spendraweshadow", "\"f80892e78c30a393ef4ab4d5a9d5a2989de6ebc7b976b241948c7f489ad716a2\", \"a4fd4d7248e6a51f1d877ddd2a4965996154acc6b8de5aa6c83d4775b283b600\", 100, \"xxx\""));
    ...
    libeshadow::CoinDenomination denom = libeshadow::IntToEshadowDenomination(denom_int);
    ...
    libeshadow::PrivateCoin coin(consensus.Eshadow_Params(false), denom, serial, randomness);
    coin.setPrivKey(privkey);
    coin.setVersion(libeshadow::PrivateCoin::CURRENT_VERSION);

    // Create the mint associated with this coin
    CEshadowMint mint(denom, coin.getPublicCoin().getValue(), randomness, serial, false, CEshadowMint::CURRENT_VERSION, &privkey);
    ...
    std::vector<CEshadowMint> vMintsSelected = {mint};
    return DoEzeeSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
