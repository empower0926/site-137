        ...
        if (zwalletMain) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read eZEE seed from DB. Wallet is probably corrupt.");
                }
...
std::set<uint256> CWallet::GetConflicts(const uint256& txid) const
{
    ...
    for (const CTxIn& txin : wtx.vin) {
        if (mapTxSpends.count(txin.prevout) <= 1 || wtx.HasEshadowSpendInputs())
...
{
    {
        ...
        if (pblock && !tx.HasEshadowSpendInputs() && !tx.IsCoinBase()) {
...
void CWallet::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    // recomputed, also:
    for (const CTxIn& txin : tx.vin) {
        if (!txin.IsEshadowSpend() && mapWallet.count(txin.prevout.hash))
            mapWallet[txin.prevout.hash].MarkDirty();
    }
}
    ...
    const isminefilter& filter) const
{
    ...
    // Sent/received.
    bool hasEshadowSpends = HasEshadowSpendInputs();
    for (unsigned int i = 0; i < vout.size(); ++i) {
        const CTxOut& txout = vout[i];
        ...
        if (nDebit > 0) {
            ...
            if (pwallet->IsChange(txout))
                continue;
        } else if (!(fIsMine & filter) && !hasEshadowSpends)
            continue;
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckEZEE = GetBoolArg("-zapwallettxes", false);
    if (fCheckEZEE)
        ezeeTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if eZEE upgrade is active.
            doEZeeRescan(pindex, block, setAddedToWallet, consensus, fCheckEZEE);
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != 10000 * COIN) {
        strError = "Invalid collateral tx value, must be 10,000 OZTG";
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == STAKEABLE_COINS && pcoin->vout[i].IsEshadowMint()) continue;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(CRecipient{scriptChange, BUDGET_FEE_TX_OLD, false}); // Old 50 OZTG collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<CRecipient> vecSend;
    vecSend.push_back(CRecipient{scriptChange, BUDGET_FEE_TX, false}); // New 5 OZTG collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-ozeety-address
        ...
        )
{
    ...
    // Parse utxos into COztgStakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<COztgStake> input(new COztgStake());
    ...
    // Sign for OZTG
    int nIn = 0;
    if (!txNew.vin[0].scriptSig.IsEshadowSpend()) {
        ...
        for (const CTxOut& out : txNew.vout) {
            ...
            libeshadow::PublicCoin pubcoin(consensus.Eshadow_Params(false));
...
CWallet::CommitResult CWallet::CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, std::string strCommand)
{
    CommitResult res;
    {
        ...
        LogPrintf("%s:\n%s", __func__, wtxNew.ToString());
        {
            ...
            if (!wtxNew.HasEshadowSpendInputs()) {
