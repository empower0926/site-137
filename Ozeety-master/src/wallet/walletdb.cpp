...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    util::ThreadRename("ozeety-wallet");
...
bool CWalletDB::WriteEshadowSpendSerialEntry(const CEshadowSpend& eshadowSpend)
{
    return Write(std::make_pair(std::string("zcserial"), eshadowSpend.GetSerial()), eshadowSpend, true);
}
bool CWalletDB::EraseEshadowSpendSerialEntry(const CBigNum& serialEntry)
{
    return Erase(std::make_pair(std::string("zcserial"), serialEntry));
}

bool CWalletDB::ReadEshadowSpendSerialEntry(const CBigNum& bnSerial)
{
    CEshadowSpend spend;
...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(std::make_pair(std::string("dezee"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(std::make_pair(std::string("dezee"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(std::make_pair(std::string("dezee"), hashPubcoin));
}

bool CWalletDB::WriteEshadowMint(const CEshadowMint& eshadowMint)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << eshadowMint.GetValue();
    uint256 hash = Hash(ss.begin(), ss.end());

    Erase(std::make_pair(std::string("eshadow"), hash));
    return Write(std::make_pair(std::string("eshadow"), hash), eshadowMint, true);
}

bool CWalletDB::ReadEshadowMint(const CBigNum &bnPubCoinValue, CEshadowMint& eshadowMint)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << bnPubCoinValue;
    uint256 hash = Hash(ss.begin(), ss.end());

    return ReadEshadowMint(hash, eshadowMint);
}

bool CWalletDB::ReadEshadowMint(const uint256& hashPubcoin, CEshadowMint& mint)
{
    return Read(std::make_pair(std::string("eshadow"), hashPubcoin), mint);
}

bool CWalletDB::EraseEshadowMint(const CEshadowMint& eshadowMint)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << eshadowMint.GetValue();
    uint256 hash = Hash(ss.begin(), ss.end());

    return Erase(std::make_pair(std::string("eshadow"), hash));
}

bool CWalletDB::ArchiveMintOrphan(const CEshadowMint& eshadowMint)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << eshadowMint.GetValue();
    uint256 hash = Hash(ss.begin(), ss.end());;

    if (!Write(std::make_pair(std::string("zco"), hash), eshadowMint)) {
        LogPrintf("%s : failed to database orphaned eshadow mint\n", __func__);
        return false;
    }

    if (!Erase(std::make_pair(std::string("eshadow"), hash))) {
        LogPrintf("%s : failed to erase orphaned eshadow mint\n", __func__);
        return false;
    }
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(std::make_pair(std::string("dezee"), dMint.GetPubcoinHash())))
...
bool CWalletDB::UnarchiveEshadowMint(const uint256& hashPubcoin, CEshadowMint& mint)
{
    if (!Read(std::make_pair(std::string("zco"), hashPubcoin), mint))
        return error("%s: failed to retrieve eshadowmint from archive", __func__);

    if (!WriteEshadowMint(mint))
        return error("%s: failed to write eshadowmint", __func__);

    uint256 hash = GetPubCoinHash(mint.GetValue());
    if (!Erase(std::make_pair(std::string("zco"), hash)))
        return error("%s : failed to erase archived eshadow mint", __func__);
...
bool CWalletDB::WriteEZEESeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseEZEESeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteEZEESeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseEZEESeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadEZEESeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadEZEESeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteEZEECount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadEZEECount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(std::string("dezee"), UINT256_ZERO);
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
...
std::list<CEshadowMint> CWalletDB::ListMintedCoins()
{
    std::list<CEshadowMint> listPubCoin;
    Dbc* pcursor = GetCursor();
    if (!pcursor)
        throw std::runtime_error(std::string(__func__)+" : cannot create DB cursor");
    unsigned int fFlags = DB_SET_RANGE;
    std::vector<CEshadowMint> vOverWrite;
    std::vector<CEshadowMint> vArchive;
    for (;;)
    {
        // Read next record
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(std::string("eshadow"), UINT256_ZERO);
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
...
std::list<CEshadowSpend> CWalletDB::ListSpentCoins()
{
    std::list<CEshadowSpend> listCoinSpend;
    Dbc* pcursor = GetCursor();
    if (!pcursor)
        throw std::runtime_error(std::string(__func__)+" : cannot create DB cursor");
    unsigned int fFlags = DB_SET_RANGE;
    for (;;)
    {
        ...
        CEshadowSpend eshadowSpendItem;
        ssValue >> eshadowSpendItem;

        listCoinSpend.push_back(eshadowSpendItem);
    }
...
std::list<CBigNum> CWalletDB::ListSpentCoinsSerial()
{
    ...
    std::list<CEshadowSpend> listCoins = ListSpentCoins();
...
std::list<CEshadowMint> CWalletDB::ListArchivedEshadows()
{
