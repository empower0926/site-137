...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            bool hasEshadowSpends = tx.HasEshadowSpendInputs();
            if (hasEshadowSpends)
                nTotalIn = tx.GetEshadowSpent();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // eZEE spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            if (!tx.HasEshadowSpendInputs() && fSortedByFee && (dPriorityDelta <= 0) && (nFeeDelta <= 0) && (feeRate < ::minRelayTxFee) && (nBlockSize + nTxSize >= nBlockMinSize))
                continue;
            ...
            // double check that there are no double spent eZEE spends in this block or tx
            if (tx.HasEshadowSpendInputs()) {
                ...
                for (const CTxIn& txIn : tx.vin) {
                    ...
                    if (txIn.IsEshadowSpend() || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            ...
                            libeshadow::CoinSpend spendObj = TxInToEshadowSpend(txIn);
                            spend = &spendObj;
                        }
                ...
                //This eZEE serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("OZEETYMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("OZEETYMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("OZEETYMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ozeety-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OZEETYMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("OZEETYMiner exception");
    } catch (...) {
        LogPrintf("OZEETYMiner exception");
    }

    LogPrintf("OZEETYMiner exiting\n");
}
