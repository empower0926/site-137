...
#ifndef OZEETY_ESHADOW_H
#define OZEETY_ESHADOW_H
...
{
private:
    libeshadow::CoinDenomination denomination;
    int nHeight;
    CBigNum value;
    CBigNum randomness;
...
public:
    ...
    CEshadowMint(libeshadow::CoinDenomination denom, const CBigNum& value, const CBigNum& randomness, const CBigNum& serialNumber, bool isUsed, const uint8_t& nVersion, CPrivKey* privkey = nullptr)
    {
        SetNull();
    ...
    libeshadow::CoinDenomination GetDenomination() const { return denomination; }
    int64_t GetDenominationAsAmount() const { return denomination * COIN; }
    void SetDenomination(libeshadow::CoinDenomination denom){ this->denomination = denom; }
    ...
    inline bool operator <(const CEshadowMint& a) const { return GetHeight() < a.GetHeight(); }

    CEshadowMint(const CEshadowMint& other) {
    ...
    bool operator == (const CEshadowMint& other) const
    {
        return this->GetValue() == other.GetValue();
    }
    
    // Copy another CEshadowMint
    inline CEshadowMint& operator=(const CEshadowMint& other) {
        denomination = other.GetDenomination();
        nHeight = other.GetHeight();
        value = other.GetValue();
        randomness = other.GetRandomness();
...
public:
    ...
    CEshadowSpend(CBigNum coinSerial, uint256 hashTx, CBigNum pubCoin, libeshadow::CoinDenomination denomination, unsigned int nAccumulatorChecksum)
    {
...
public:
    void AddSpend(const CEshadowSpend& spend);
    std::vector<CEshadowSpend> GetSpends();
