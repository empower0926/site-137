...
CeZEEWallet::CeZEEWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing ezee seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadEZEESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseEZEESeed_deprecated()) {
                    LogPrintf("%s: Updated eZEE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ezee seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ezee wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CeZEEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteEZEECount(nCountLastUsed);
    else if (!walletdb.ReadEZEECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CeZEEWallet::Lock()
{
    seedMaster.SetNull();
}

void CeZEEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CeZEEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        uint512 seedEshadow = GetEshadowSeed(i);
        CBigNum bnValue;
        CBigNum bnSerial;
        CBigNum bnRandomness;
        CKey key;
        SeedToEZEE(seedEshadow, bnValue, bnSerial, bnRandomness, key);
...
bool CeZEEWallet::LoadMintPoolFromDB()
{
...
void CeZEEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CeZEEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CeZEEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->ezeeTracker->HasPubcoinHash(pMint.first)) {
            ...
            uint256 txHash;
            CEshadowMint mint;
            if (eshadowDB->ReadCoinMint(pMint.first, txHash)) {
                ...
                libeshadow::CoinDenomination denomination = libeshadow::CoinDenomination::ZQ_ERROR;
                ...
                if (!fFoundMint || denomination == libeshadow::ZQ_ERROR) {
...
bool CeZEEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libeshadow::CoinDenomination& denom)
{
    ...
    // Regenerate the mint
    uint512 seedEshadow = GetEshadowSeed(pMint.second);
    CBigNum bnValueGen;
    CBigNum bnSerial;
    CBigNum bnRandomness;
    CKey key;
    SeedToEZEE(seedEshadow, bnValueGen, bnSerial, bnRandomness, key);
    ...
    CDeterministicMint dMint(libeshadow::PrivateCoin::CURRENT_VERSION, pMint.second, hashSeed, hashSerial, hashPubcoin, hashStake);
    ...
    // Add to ezeeTracker which also adds to database
    wallet->ezeeTracker->Add(dMint, true);
...
void CeZEEWallet::SeedToEZEE(const uint512& seedEshadow, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libeshadow::EshadowParams* params = Params().GetConsensus().Eshadow_Params(false);

    //convert state seed into a seed for the private key
    uint256 nSeedPrivKey = seedEshadow.trim256();

    bool isValidKey = false;
    key = CKey();
    while (!isValidKey) {
        nSeedPrivKey = Hash(nSeedPrivKey.begin(), nSeedPrivKey.end());
        isValidKey = libeshadow::GenerateKeyPair(params->coinCommitmentGroup.groupOrder, nSeedPrivKey, key, bnSerial);
    }

    //hash randomness seed with Bottom 256 bits of seedEshadow & attempts256 which is initially 0
    uint256 randomnessSeed = uint512(seedEshadow >> 256).trim256();
...
uint512 CeZEEWallet::GetEshadowSeed(uint32_t n)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << seedMaster << n;
    uint512 eshadowSeed = Hash512(ss.begin(), ss.end());
    return eshadowSeed;
}

void CeZEEWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(wallet->strWalletFile);
    walletdb.WriteEZEECount(nCountLastUsed);
...
void CeZEEWallet::GenerateDeterministicEZEE(libeshadow::CoinDenomination denom, libeshadow::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
    ...
    //LogPrintf("%s : Generated new deterministic mint. Count=%d pubcoin=%s seed=%s\n", __func__, nCount, coin.getPublicCoin().getValue().GetHex().substr(0,6), seedEshadow.GetHex().substr(0, 4));
}

void CeZEEWallet::GenerateMint(const uint32_t& nCount, const libeshadow::CoinDenomination denom, libeshadow::PrivateCoin& coin, CDeterministicMint& dMint)
{
    uint512 seedEshadow = GetEshadowSeed(nCount);
    CBigNum bnValue;
    CBigNum bnSerial;
    CBigNum bnRandomness;
    CKey key;
    SeedToEZEE(seedEshadow, bnValue, bnSerial, bnRandomness, key);
    coin = libeshadow::PrivateCoin(Params().GetConsensus().Eshadow_Params(false), denom, bnSerial, bnRandomness);
    coin.setPrivKey(key.GetPrivKey());
    coin.setVersion(libeshadow::PrivateCoin::CURRENT_VERSION);
...
bool CeZEEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CeZEEWallet::RegenerateMint(const CDeterministicMint& dMint, CEshadowMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
    ...
    //Generate the coin
    libeshadow::PrivateCoin coin(Params().GetConsensus().Eshadow_Params(false), dMint.GetDenomination(), false);
    CDeterministicMint dMintDummy;
    GenerateMint(dMint.GetCount(), dMint.GetDenomination(), coin, dMintDummy);

    //Fill in the eshadowmint object's details
