...
CeZEETracker::CeZEETracker(CWallet* parent)
{
    this->wallet = parent;
...
CeZEETracker::~CeZEETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CeZEETracker::Init()
{
    //Load all CEshadowMints and CDeterministicMints from the database
    if (!fInitialized) {
...
bool CeZEETracker::Archive(CMintMeta& meta)
{
    ...
    CEshadowMint mint;
    if (walletdb.ReadEshadowMint(meta.hashPubcoin, mint)) {
        if (!CWalletDB(wallet->strWalletFile).ArchiveMintOrphan(mint))
            return error("%s: failed to archive eshadowmint", __func__);
    } else {
...
bool CeZEETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
        ...
        CEshadowMint mint;
        if (!walletdb.UnarchiveEshadowMint(hashPubcoin, mint))
            return error("%s: failed to unarchiveeshadow mint", __func__);
        Add(mint, false);
    }
...
CMintMeta CeZEETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CeZEETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CeZEETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CeZEETracker::GetSerialHashes()
{
...
CAmount CeZEETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
    //! eshadow specific fields
    std::map<libeshadow::CoinDenomination, unsigned int> myEshadowSupply;
    for (auto& denom : libeshadow::eshadowDenomList) {
        myEshadowSupply.insert(std::make_pair(denom, 0));
    }

    {
        ...
        for (auto& it : mapSerialHashes) {
            ...
            nTotal += libeshadow::EshadowDenominationToAmount(meta.denom);
            myEshadowSupply.at(meta.denom)++;
        }
    }
...
CAmount CeZEETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CeZEETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CeZEETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CeZEETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CeZEETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CeZEETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CeZEETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CeZEETracker::UpdateEshadowMint(const CEshadowMint& mint)
{
    ...
    //Write to db
    return CWalletDB(wallet->strWalletFile).WriteEshadowMint(mint);
}

bool CeZEETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
    ...
    if (meta.isDeterministic) {
        ...
        if (!walletdb.WriteEshadowMint(mint))
...
void CeZEETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CeZEEWallet* eZEEWallet)
{
    bool iseZEEWalletInitialized = (nullptr != eZEEWallet);
    CMintMeta meta;
    ...
    if (!iseZEEWalletInitialized)
        eZEEWallet = new CeZEEWallet(wallet);
    meta.isSeedCorrect = eZEEWallet->CheckSeed(dMint);
    if (!iseZEEWalletInitialized)
        delete eZEEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CeZEETracker::Add(const CEshadowMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
    meta.hashPubcoin = GetPubCoinHash(mint.GetValue());
    meta.nHeight = mint.GetHeight();
    meta.nVersion = libeshadow::ExtractVersionFromSerial(mint.GetSerialNumber());
    ...
    if (isNew)
        CWalletDB(wallet->strWalletFile).WriteEshadowMint(mint);
}

void CeZEETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CeZEETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CeZEETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CeZEETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
    //! Check whether this mint has been spent and is considered 'pending' or 'confirmed'
    // If there is not a record of the block height, then look it up and assign it
    uint256 txidMint;
    bool isMintInChain = eshadowDB->ReadCoinMint(mint.hashPubcoin, txidMint);
    ...
    uint256 txidSpend;
    bool isConfirmedSpend = eshadowDB->ReadCoinSpend(mint.hashSerial, txidSpend);
    ...
    if (!mint.nHeight || !isMintInChain || isUsed != mint.isUsed) {
        ...
        if (mint.txid.IsNull()) {
            if (!isMintInChain) {
                LogPrintf("%s : Failed to find mint in eshadowDB %s\n", __func__, mint.hashPubcoin.GetHex().substr(0, 6));
...
std::set<CMintMeta> CeZEETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        std::list<CEshadowMint> listMintsDB = walletdb.ListMintedCoins();
        for (auto& mint : listMintsDB)
            Add(mint);
        LogPrint(BCLog::LEGACYZC, "%s: added %d eshadowmints from DB\n", __func__, listMintsDB.size());
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dezee from DB\n", __func__, listDeterministicDB.size());
    }
