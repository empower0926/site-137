...
// Copyright (c) 2015-2020 The OZEETY developers
...
#if defined(NDEBUG)
#error "OZEETY cannot be compiled without assertions."
#endif
        ...
        bool* pfMissingInputs, bool fOverrideMempoolLimit, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    //Temporarily disable eshadow for maintenance
    if (sporkManager.IsSporkActive(SPORK_16_ESHADOW_MAINTENANCE_MODE) && tx.ContainsEshadows())
        return state.DoS(10, error("%s : Eshadow transactions are temporarily disabled for maintenance",
    ...
    bool hasZcSpendInputs = tx.HasEshadowSpendInputs();
...

    {
        ...
        CAmount nValueIn = 0;
        if (hasZcSpendInputs) {
            if (!AcceptToMemoryPoolEshadow(tx, nValueIn, chainHeight, state, consensus)) {
                return false;
            }
        } else {
            LOCK(pool.cs);
            ...
            // Reject legacy eZEE mints
            if (!Params().IsRegTestNet() && tx.HasEshadowMintOutputs())
                return state.Invalid(error("%s : tried to include eZEE mint output in tx %s",
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
...

    {
        ...
        if (isDSTX) {
            ...
            CAmount txMinFee = GetMinRelayFee(tx, pool, nSize, true);
            if (fLimitFree && nFees < txMinFee && !tx.HasEshadowSpendInputs())
            ...
            // This mitigates 'penny-flooding' -- sending thousands of free transactions just to
            // be annoying or make others' transactions take longer to confirm.
            if (fLimitFree && nFees < ::minRelayTxFee.GetFee(nSize) && !tx.HasEshadowSpendInputs()) {
                static RecursiveMutex csFreeLimiter;
                static double dFreeCount;
                static int64_t nLastTime;
                int64_t nNow = GetTime();
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight == 0) {
        nSubsidy = 672175000 * COIN;
    } else if (nHeight <= 265000 && nHeight > 0) {
        nSubsidy = 25 * COIN;
    } else if (nHeight <= 1000000 && nHeight >= 265001) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 1800000 && nHeight >= 1000001) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 2500000 && nHeight >= 1800001) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 6000000 && nHeight >= 2500001) {
        nSubsidy = 5 * COIN;
    } else {
        nSubsidy = 1 * COIN;
    }
...
{
    // mark inputs spent
    if (!tx.IsCoinBase() && !tx.HasEshadowSpendInputs()) {
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsEshadowSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    libeshadow::CoinSpend spendObj = TxInToEshadowSpend(in);
                    spend = &spendObj;
                }
                ...
                if (!spend->HasValidSerial(params)) {
                    ...
                    if (eshadowDB->ReadCoinSpend(bnActualSerial, txHash)) {
                    ...
                    //Record all txouts from this invalid eshadow spend tx as invalid
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    //Track eZEE money supply
    if (!UpdateEZEESupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new eZEE supply", __func__);

    // undo transactions in reverse order
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        const CTransaction& tx = block.vtx[i];

        if (!DisconnectEshadowTx(tx, nValueIn, eshadowDB))
            return false;
        ...
        // restore inputs
        if (!tx.IsCoinBase() && !tx.HasEshadowSpendInputs()) { // not coinbases or eshadowspend because they dont have traditional inputs
        ...
        if (!tx.HasEshadowSpendInputs() && !tx.IsCoinBase() && view.HaveInputs(tx))
            nValueIn += view.GetValueIn(tx);
    }
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
        ...
        // Legacy Eshadow DB: If Accumulators Checkpoint is changed, remove changed checksums
...
{
    util::ThreadRename("OZEETY-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    std::vector<std::pair<libeshadow::CoinSpend, uint256> > vSpends;
    std::vector<std::pair<libeshadow::PublicCoin, uint256> > vMints;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        //Temporarily disable eshadow transactions for maintenance
        if (block.nTime > sporkManager.GetSporkValue(SPORK_16_ESHADOW_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsEshadows()) {
            return state.DoS(100, error("ConnectBlock() : eshadow transactions are currently in maintenance mode"));
        }
        ...
        if (tx.HasEshadowMintOutputs()) {
            ...
            for (auto& out : tx.vout) {
                if (!out.IsEshadowMint()) continue;
                libeshadow::PublicCoin coin(consensus.Eshadow_Params(false));
                if (!TxOutToPublicCoin(out, coin, state))
                    return state.DoS(100, error("%s: failed final check of eshadowmint for tx %s", __func__, tx.GetHash().GetHex()));
        ...
        if (tx.HasEshadowSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                bool isPrivEshadowSpend = txIn.IsEshadowSpend();
                if (!isPrivEshadowSpend && !isPublicSpend)
                    continue;
                ...
                if (isPublicSpend) {
                    ...
                    if (!ContextualCheckEshadowSpend(tx, &publicSpend, pindex->nHeight, hashBlock))
                    ...
                    if (!ContextualCheckEshadowSpend(tx, &spend, pindex->nHeight, hashBlock))
                        return state.DoS(100, error("%s: failed to add block %s with invalid eshadowspend", __func__, tx.GetHash().GetHex()), REJECT_INVALID);
                }
            }
    ...
    if (pwalletMain) {
        ...
        for (const std::pair<libeshadow::CoinSpend, uint256>& pSpend : vSpends) {
            // Send signal to wallet if this is ours
            if (pwalletMain->IsMyEshadowSpend(pSpend.first.getCoinSerialNumber())) {
                LogPrintf("%s: %s detected eshadowspend in transaction %s \n", __func__,
                          pSpend.first.getCoinSerialNumber().GetHex(), pSpend.second.GetHex());
                pwalletMain->NotifyEshadowChanged(pwalletMain, pSpend.first.getCoinSerialNumber().GetHex(), "Used",
                                                   CT_UPDATED);
    ...
    if (!vSpends.empty() && !eshadowDB->WriteCoinSpendBatch(vSpends))
        return AbortNode(state, "Failed to record coin serials to database");

    if (!vMints.empty() && !eshadowDB->WriteCoinMintBatch(vMints))
    ...
    // Update eZEE money supply map
    if (!UpdateEZEESupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new eZEE supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the eZEE supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
            && GetEshadowSupply() != consensus.ZC_WrappedSerialsSupply + GetWrapppedSerialInflationAmount()) {
        RecalculateOZTGSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC_V2) &&
            pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        // Legacy Eshadow DB: If Accumulators Checkpoint is changed, database the checksums
    ...
    try {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                // Flush eshadow supply
                if (!mapEshadowSupply.empty() && !eshadowDB->WriteZCSupply(mapEshadowSupply)) {
                    return AbortNode(state, "Failed to write eshadow supply to DB");
                }
                // Flush money supply
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Eshadow activation
    bool fEshadowActive = block.GetBlockTime() > Params().GetConsensus().ZC_TimeStart;
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent eZEE spends in this block
        if (tx.HasEshadowSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsEshadowPublicSpend();
                if (txIn.IsEshadowSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if (!IsInitialBlockDownload() && !CheckPublicCoinSpendVersion(spend.getVersion())) {
                            return state.DoS(0, error("%s : Public Eshadow spend version %d not accepted. must be version %d.",
                                    __func__, spend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-zcspend-version");
                        }
                    } else {
                        spend = TxInToEshadowSpend(txIn);
                    }
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of eZEE serial %s in block\n Block: %s",
    ...
    unsigned int nMaxBlockSigOps = fEshadowActive ? MAX_BLOCK_SIGOPS_CURRENT : MAX_BLOCK_SIGOPS_LEGACY;
    if (nSigOps > nMaxBlockSigOps)
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // OZEETY Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nOzeetyBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nOzeetyBadBlockBits)) {
            // accept OZEETY block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> oztgInputs;
        std::vector<CTxIn> eZEEInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsEshadowSpend()){
                eZEEInputs.push_back(stakeIn);
            }else{
                oztgInputs.push_back(stakeIn);
            }
        }
        const bool hasOZTGInputs = !oztgInputs.empty();
        const bool hasEZEEInputs = !eZEEInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                if(consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC)) {
                    bool isPublicSpend = in.IsEshadowPublicSpend();
                    bool isPrivEshadowSpend = in.IsEshadowSpend();
                    if (isPrivEshadowSpend || isPublicSpend) {
                ...
                if(hasOZTGInputs) {
                    ...
                    for (const CTxIn& oztgIn : oztgInputs)
                        if(oztgIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        // If this input is a eshadow spend, and the coinstake has eshadow inputs
                        // then store the serials for later check
                        if(hasEZEEInputs && in.IsEshadowSpend())
                            vBlockSerials.push_back(TxInToEshadowSpend(in).getCoinSerialNumber());

                        // Loop through every input of the staking tx
                        if (hasOZTGInputs) {
                            for (const CTxIn& stakeIn : oztgInputs)
            ...
            // Now that this loop if completed. Check if we have eZEE inputs.
            if(hasEZEEInputs) {
                for (const CTxIn& eZeeInput : eZEEInputs) {
                    libeshadow::CoinSpend spend = TxInToEshadowSpend(eZeeInput);
                    ...
                    if (!ContextualCheckEshadowSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckEshadowSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-ezee");
        ...
        if(!stakeTxIn.HasEshadowSpendInputs()) {
            ...
            if(!isBlockFromFork)
                for (const CTxIn& eZeeInput : eZEEInputs) {
                        libeshadow::CoinSpend spend = TxInToEshadowSpend(eZeeInput);
                        if (!ContextualCheckEshadowSpend(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                            return state.DoS(100,error("%s: main chain ContextualCheckEshadowSpend failed for tx %s", __func__,
                                    stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-ezee");
                }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // OZEETY: We use certain sporks during IBD, so check to see if they are
        // available. If not, ask the first peer connected for them.
        // TODO: Move this to an instant broadcast of the sporks.
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_14_NEW_PROTOCOL_ENFORCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) ||
                              !pSporkDB->SporkExists(SPORK_16_ESHADOW_MAINTENANCE_MODE) ||
                              !pSporkDB->SporkExists(SPORK_17_COLDSTAKING_ENFORCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_18_ESHADOW_PUBLICSPEND_V4);
        ...
        bool fMissingEshadowInputs = false;
        CValidationState state;

        mapAlreadyAskedFor.erase(inv);

        if (!tx.HasEshadowSpendInputs() && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs, false, ignoreFees)) {
            mempool.check(pcoinsTip);
            RelayTransaction(tx);
        ...
        } else if (tx.HasEshadowSpendInputs() && AcceptToMemoryPool(mempool, state, tx, true, &fMissingEshadowInputs, false, false, ignoreFees)) {
            //Presstab: ZCoin has a bunch of code commented out here. Is this something that should have more going on?
            //Also there is nothing that handles fMissingEshadowInputs. Does there need to be?
            RelayTransaction(tx);
            LogPrint(BCLog::MEMPOOL, "AcceptToMemoryPool: Eshadowspend peer=%d %s : accepted %s (poolsz %u)\n",
