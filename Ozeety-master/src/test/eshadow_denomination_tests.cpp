...
#include "test/test_ozeety.h"
#include <boost/test/unit_test.hpp>
#include <iostream>


BOOST_FIXTURE_TEST_SUITE(eshadow_denom_tests, BasicTestingSetup)

//translation from ozeety quantity to eshadow denomination
BOOST_AUTO_TEST_CASE(amount_to_denomination_test)
{
    ...
    BOOST_CHECK_MESSAGE(libeshadow::AmountToEshadowDenomination(amount) == libeshadow::ZQ_ONE, "For COIN denomination should be ZQ_ONE");

    //valid amount (max edge)
    CAmount amount1 = 5000 * COIN;
    BOOST_CHECK_MESSAGE(libeshadow::AmountToEshadowDenomination(amount1) == libeshadow::ZQ_FIVE_THOUSAND, "For 5000*COIN denomination should be ZQ_ONE");

    //invalid amount (too much)
    CAmount amount2 = 7000 * COIN;
    BOOST_CHECK_MESSAGE(libeshadow::AmountToEshadowDenomination(amount2) == libeshadow::ZQ_ERROR, "For 7000*COIN denomination should be Invalid -> ZQ_ERROR");

    //invalid amount (not enough)
    CAmount amount3 = 1;
    BOOST_CHECK_MESSAGE(libeshadow::AmountToEshadowDenomination(amount3) == libeshadow::ZQ_ERROR, "For 1 denomination should be Invalid -> ZQ_ERROR");
}

BOOST_AUTO_TEST_CASE(denomination_to_value_test)
{
    std::cout << "Running EshadowDenominationToValue_test...\n";

    int64_t Value = 1 * COIN;
    libeshadow::CoinDenomination denomination = libeshadow::ZQ_ONE;
    BOOST_CHECK_MESSAGE(libeshadow::EshadowDenominationToAmount(denomination) == Value, "Wrong Value - should be 1");

    Value = 10 * COIN;
    denomination = libeshadow::ZQ_TEN;
    BOOST_CHECK_MESSAGE(libeshadow::EshadowDenominationToAmount(denomination) == Value, "Wrong Value - should be 10");

    Value = 50 * COIN;
    denomination = libeshadow::ZQ_FIFTY;
    BOOST_CHECK_MESSAGE(libeshadow::EshadowDenominationToAmount(denomination) == Value, "Wrong Value - should be 50");

    Value = 500 * COIN;
    denomination = libeshadow::ZQ_FIVE_HUNDRED;
    BOOST_CHECK_MESSAGE(libeshadow::EshadowDenominationToAmount(denomination) == Value, "Wrong Value - should be 500");

    Value = 100 * COIN;
    denomination = libeshadow::ZQ_ONE_HUNDRED;
    BOOST_CHECK_MESSAGE(libeshadow::EshadowDenominationToAmount(denomination) == Value, "Wrong Value - should be 100");

    Value = 0 * COIN;
    denomination = libeshadow::ZQ_ERROR;
    BOOST_CHECK_MESSAGE(libeshadow::EshadowDenominationToAmount(denomination) == Value, "Wrong Value - should be 0");
}

BOOST_AUTO_TEST_CASE(eshadow_spend_test241)
{
    const int nMaxNumberOfSpends = 4;
    const int DenomAmounts[] = {1, 2, 3, 4, 0, 0, 0, 0};
    CAmount nSelectedValue;
    std::list<CMintMeta> listMints;
    std::map<libeshadow::CoinDenomination, CAmount> mapDenom;
    ...
    for (const auto& denom : libeshadow::eshadowDenomList) {
        for (int i = 0; i < DenomAmounts[j]; i++) {
            CAmount currentAmount = libeshadow::EshadowDenominationToAmount(denom);
        ...
        mapDenom.insert(std::pair<libeshadow::CoinDenomination, CAmount>(denom, DenomAmounts[j]));
        j++;
    }
    ...
    j = 0;
    for (const auto& denom : libeshadow::eshadowDenomList)
        std::cout << DenomAmounts[j++] << "*" << libeshadow::EshadowDenominationToAmount(denom) / COIN << " + ";
    std::cout << "\n";
    ...
    CAmount OneCoinAmount = libeshadow::EshadowDenominationToAmount(libeshadow::ZQ_ONE);
...
BOOST_AUTO_TEST_CASE(eshadow_spend_test115)
{
    const int nMaxNumberOfSpends = 4;
...
BOOST_AUTO_TEST_CASE(eshadow_spend_test_from_245)
{
    ...
    CAmount nSelectedValue;
    std::list<CMintMeta> listMints;
    std::map<libeshadow::CoinDenomination, CAmount> mapOfDenomsHeld;
    ...
    for (const auto& denom : libeshadow::eshadowDenomList) {
        ...
        mapOfDenomsHeld.insert(std::pair<libeshadow::CoinDenomination, CAmount>(denom, DenomAmounts[j]));
        j++;
    }
    CoinsHeld = nTotalAmount / COIN;
    std::cout << "Curremt Amount held = " << CoinsHeld << ": ";
...
BOOST_AUTO_TEST_CASE(eshadow_spend_test_from_145)
{
    const int nMaxNumberOfSpends = 5;
    // CoinsHeld = 145
...
BOOST_AUTO_TEST_CASE(eshadow_spend_test99)
{
    const int nMaxNumberOfSpends = 4;
    const int DenomAmounts[] = {0, 1, 4, 2, 1, 0, 0, 0};
    CAmount nSelectedValue;
