...
BOOST_FIXTURE_TEST_SUITE(eshadow_transactions_tests, WalletTestingSetup)

BOOST_AUTO_TEST_CASE(eshadow_spend_test)
{
    SelectParams(CBaseChainParams::MAIN);
    libeshadow::EshadowParams *ZCParams = Params().GetConsensus().Eshadow_Params(false);
    (void)ZCParams;

    bool fFirstRun;
    pwalletMain->LoadWallet(fFirstRun);
    pwalletMain->ezeeTracker = std::unique_ptr<CeZEETracker>(new CeZEETracker(pwalletMain));
    CMutableTransaction tx;
    CWalletTx* wtx = new CWalletTx(pwalletMain, tx);
    std::vector<CEshadowSpend> vSpends;
    std::vector<CEshadowMint> vMints;
    CAmount nAmount = COIN;

    CEshadowSpendReceipt receipt;
    std::list<std::pair<CTxDestination, CAmount>> outputs;
    pwalletMain->SpendEshadow(nAmount, *wtx, receipt, vMints, outputs);

    BOOST_CHECK_MESSAGE(receipt.GetStatus() == EZEE_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));

    nAmount = 1;
    CEshadowSpendReceipt receipt2;
    pwalletMain->SpendEshadow(nAmount, *wtx, receipt2, vMints, outputs);
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == EZEE_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));

}

BOOST_AUTO_TEST_CASE(eshadow_schnorr_signature_test)
{
    const int NUM_OF_TESTS = 50;
    SelectParams(CBaseChainParams::MAIN);
    libeshadow::EshadowParams *ZCParams_v1 = Params().GetConsensus().Eshadow_Params(true);
    (void)ZCParams_v1;
    libeshadow::EshadowParams *ZCParams_v2 = Params().GetConsensus().Eshadow_Params(false);
    (void)ZCParams_v2;
    ...
    for (int i=0; i<NUM_OF_TESTS; i++) {
        ...
        for (uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
            if (c.isPrime(ESHADOW_MINT_PRIME_PARAM)) break;
        ...
        BOOST_CHECK_MESSAGE(c.isPrime(ESHADOW_MINT_PRIME_PARAM), "Unable to mint v1 coin");
        libeshadow::PrivateCoin privCoin_v1(ZCParams_v1, libeshadow::CoinDenomination::ZQ_ONE, s, r);
        ...
        // mint a v2 coin
        libeshadow::PrivateCoin privCoin_v2(ZCParams_v2, libeshadow::CoinDenomination::ZQ_ONE, true);
        ...
        libeshadow::CoinRandomnessSchnorrSignature crss_v1(ZCParams_v1, randomness_v1, msghash);
        CDataStream ser_crss_v1(SER_NETWORK, PROTOCOL_VERSION);
        ser_crss_v1 << crss_v1;

        // sign the msghash with the randomness of the v2 coin
        libeshadow::CoinRandomnessSchnorrSignature crss_v2(ZCParams_v2, randomness_v2, msghash);
        CDataStream ser_crss_v2(SER_NETWORK, PROTOCOL_VERSION);
        ser_crss_v2 << crss_v2;

        // unserialize the v1 signature into a fresh object and verify it
        libeshadow::CoinRandomnessSchnorrSignature new_crss_v1(ser_crss_v1);
        BOOST_CHECK_MESSAGE(
        ...
        libeshadow::CoinRandomnessSchnorrSignature new_crss_v2(ser_crss_v2);
        BOOST_CHECK_MESSAGE(
...
BOOST_AUTO_TEST_CASE(eshadow_public_spend_test)
{
    SelectParams(CBaseChainParams::MAIN);
    libeshadow::EshadowParams *ZCParams_v1 = Params().GetConsensus().Eshadow_Params(true);
    libeshadow::EshadowParams *ZCParams_v2 = Params().GetConsensus().Eshadow_Params(false);
    ...
    CEshadowMint mint_v1 = CEshadowMint(
    ...
    CEshadowMint mint_v2 = CEshadowMint(
    ...
    << OP_ESHADOWMINT << privCoin_v1.getPublicCoin().getValue().getvch().size() << privCoin_v1.getPublicCoin().getValue().getvch();
    CTxOut out_v1 = CTxOut(libeshadow::EshadowDenominationToAmount(privCoin_v1.getPublicCoin().getDenomination()), scriptSerializedCoin_v1);
    prevTx_v1.vout.push_back(out_v1);

    CScript scriptSerializedCoin_v2 = CScript()
    << OP_ESHADOWMINT << privCoin_v2.getPublicCoin().getValue().getvch().size() << privCoin_v2.getPublicCoin().getValue().getvch();
    CTxOut out_v2 = CTxOut(libeshadow::EshadowDenominationToAmount(privCoin_v2.getPublicCoin().getDenomination()), scriptSerializedCoin_v2);
    ...
    // -----------------------------------
    int spendVersion = 3;
    BOOST_CHECK_MESSAGE(EZEEModule::createInput(in1, mint_v2, tx1.GetHash(), spendVersion),
    ...
    BOOST_CHECK_MESSAGE(EZEEModule::validateInput(in1, out_v2, tx1, publicSpend1),
    ...
    BOOST_CHECK_MESSAGE(!EZEEModule::validateInput(in1, out_v2, tx1, publicSpend1b), "Different denomination for mint v2 and spendVersion 3");

    // check spendVersion = 4 for v2 coins
    // -----------------------------------
    spendVersion = 4;
    BOOST_CHECK_MESSAGE(EZEEModule::createInput(in2, mint_v2, tx2.GetHash(), spendVersion),
    ...
    BOOST_CHECK_MESSAGE(EZEEModule::validateInput(in2, out_v2, tx2, publicSpend2),
            "Failed to validate zc input for mint v2 and spendVersion 4");

    // Verify that it fails with a different denomination
    in2.nSequence = 500;
    PublicCoinSpend publicSpend2b(ZCParams_v2);
    BOOST_CHECK_MESSAGE(!EZEEModule::validateInput(in2, out_v2, tx2, publicSpend2b), "Different denomination for mint v2 and spendVersion 4");

    // check spendVersion = 4 for v1 coins
    // -----------------------------------
    BOOST_CHECK_MESSAGE(EZEEModule::createInput(in3, mint_v1, tx3.GetHash(), spendVersion),
    ...
    BOOST_CHECK_MESSAGE(EZEEModule::validateInput(in3, out_v1, tx3, publicSpend3),
            "Failed to validate zc input for mint v1 and spendVersion 4");

    // Verify that it fails with a different denomination
    in3.nSequence = 500;
    PublicCoinSpend publicSpend3b(ZCParams_v1);
    BOOST_CHECK_MESSAGE(!EZEEModule::validateInput(in3, out_v1, tx3, publicSpend3b), "Different denomination for mint v1 and spendVersion 4");
