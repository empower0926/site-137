...
UniValue getrawmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in ozeety\n"
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ozeety addresses\n"
...
UniValue findserial(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "findserial \"serial\"\n"
            "\nSearches the eshadow database for a eshadow spend transaction that contains the specified serial\n"

            "\nArguments:\n"
            "1. serial   (string, required) the serial of a eshadow spend to search for.\n"
    ...
    uint256 txid;
    bool fSuccess = eshadowDB->ReadCoinSpend(bnSerial, txid);
...
UniValue getserials(const JSONRPCRequest& request) {
    ...
    while (true) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            std::string spentTo = "";
            if (fVerbose) {
                if (tx.vout[0].IsEshadowMint()) {
                    spentTo = "Eshadow Mint";
                } else if (tx.vout[0].IsEmpty()) {
                    spentTo = "Eshadow Stake";
                } else {
                    txnouttype type;
                ...
                if (txin.IsEshadowSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if(!GetOutput(txin.prevout.hash, txin.prevout.n, state, prevOut)){
                            throw JSONRPCError(RPC_INTERNAL_ERROR, "public eshadow spend prev output not found");
                        }
                        ...
                        if (!EZEEModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
                            throw JSONRPCError(RPC_INTERNAL_ERROR, "public eshadow spend parse failed");
                        }
                        serial_str = publicSpend.getCoinSerialNumber().ToString(16);
                        denom = libeshadow::EshadowDenominationToInt(publicSpend.getDenomination());
                    } else {
                        libeshadow::CoinSpend spend = TxInToEshadowSpend(txin);
                        serial_str = spend.getCoinSerialNumber().ToString(16);
                        denom = libeshadow::EshadowDenominationToInt(spend.getDenomination());
                    }
                    if (!fVerbose) {
...
UniValue getblockindexstats(const JSONRPCRequest& request) {
    ...
    std::map<libeshadow::CoinDenomination, int64_t> mapSpendCount;
    std::map<libeshadow::CoinDenomination, int64_t> mapPublicSpendCount;
    ...
    while (true) {
        ...
        for (const CTransaction& tx : block.vtx) {
            if (tx.IsCoinBase() || (tx.IsCoinStake() && !tx.HasEshadowSpendInputs()))
                continue;

            // fetch input value from prevouts and count spends
            for (unsigned int j = 0; j < tx.vin.size(); j++) {
                if (tx.vin[j].IsEshadowSpend()) {
                    if (!fFeeOnly)
                        mapSpendCount[libeshadow::IntToEshadowDenomination(tx.vin[j].nSequence)]++;
                    continue;
                }
                if (tx.vin[j].IsEshadowPublicSpend()) {
                    if (!fFeeOnly)
                        mapPublicSpendCount[libeshadow::IntToEshadowDenomination(tx.vin[j].nSequence)]++;
                    continue;
                }
    ...
    if (!fFeeOnly) {
        ...
        for (auto& denom : libeshadow::eshadowDenomList) {
            spend_obj.push_back(Pair(strprintf("denom_%d", EshadowDenominationToInt(denom)), mapSpendCount[denom]));
            pubspend_obj.push_back(Pair(strprintf("denom_%d", EshadowDenominationToInt(denom)), mapPublicSpendCount[denom]));
        }
