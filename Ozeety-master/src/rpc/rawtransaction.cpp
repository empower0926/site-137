...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the ozeety address, the value is the ozeety amount\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : addrList) {
        ...
        if (!IsValidDestination(address))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid OZEETY address: ")+name_);
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) ozeety address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "   {\n"
            "     \"changeAddress\"     (string, optional, default pool address) The OZEETY address to receive the change\n"
    ...
    if (request.params.size() > 1) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(changeAddress))
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid OZEETY address");
        }
...
UniValue getspenteshadowamount(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "getspenteshadowamount hexstring index\n"
            "\nReturns value of spent eshadow output designated by transaction hash and input index.\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("getspenteshadowamount", "78021ebf92a80dfccef1413067f1222e37535399797cce029bb40ad981131706 0"));
    ...
    if (!input.IsEshadowSpend())
        return -1;

    libeshadow::CoinSpend spend = TxInToEshadowSpend(input);
    CAmount nValue = libeshadow::EshadowDenominationToAmount(spend.getDenomination());
    return FormatMoney(nValue);
}

#ifdef ENABLE_WALLET

UniValue createraweshadowspend(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "createraweshadowspend mint_input ( \"address\" )\n"
            "\nCreates raw eZEE public spend.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "}\n"
            "\nExamples\n" +
            HelpExampleCli("createraweshadowspend", "0d8c16eee7737e3cc1e4e70dc006634182b175e039700931283b202715a0818f") +
            HelpExampleRpc("createraweshadowspend", "0d8c16eee7737e3cc1e4e70dc006634182b175e039700931283b202715a0818f"));
    ...
    if (address_str != "") {
        ...
        if(!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OZEETY address");
    }
    ...
    std::vector<CEshadowMint> vMintsSelected = std::vector<CEshadowMint>(1,input_mint);
