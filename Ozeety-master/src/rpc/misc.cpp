...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total ozeety balance of the wallet (excluding eshadows)\n"
            "  \"eshadowbalance\": xxxxxxx,   (numeric) the total eshadow balance of the wallet\n"
            ...
            "  \"eZEEsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 eZEE denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 eZEE denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 eZEE denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 eZEE denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 eZEE denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 eZEE denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 eZEE denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 eZEE denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all eZEE denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in ozeety/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in ozeety/kb\n"
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("eshadowbalance", ValueFromAmount(pwalletMain->GetEshadowBalance(true))));
    ...
    UniValue ezeeObj(UniValue::VOBJ);
    for (auto denom : libeshadow::eshadowDenomList) {
        if (mapEshadowSupply.empty())
            ezeeObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            ezeeObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapEshadowSupply.at(denom) * (denom*COIN))));
    }
    ezeeObj.push_back(Pair("total", ValueFromAmount(GetEshadowSupply())));
    obj.push_back(Pair("eZEEsupply", ezeeObj));
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"ozeetyaddress\"\n"
            "\nReturn information about the given ozeety address.\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"     (string, required) The ozeety address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ozeetyaddress\",    (string) The ozeety address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for OZEETY cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: OZEETY address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ozeety addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ozeety address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"ozeetyaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"ozeetyaddress\"  (string, required) The ozeety address to use for the signature.\n"
