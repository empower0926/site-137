...
#include "config/ozeety-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    util::ThreadRename("ozeety-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ozeety.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ozeetyd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all OZEETY specific functionality (Masternodes, Eshadow, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in OZTG) allowed by GUI and RPC for the stake split threshold (default: %s)"),
    ...
    strUsage += HelpMessageGroup(_("Eshadow options:"));
    strUsage += HelpMessageOpt("-reindexeshadow=<n>", strprintf(_("Delete all eshadow spends and mints that have been recorded to the blockchain database and reindex them (0-1, default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The OZEETY Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("ozeety-loadblk");
...
/** Sanity checks
 *  Ensure that OZEETY is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Check level must be 4 for eshadow checks
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. OZEETY Core is shutting down."));
...
#endif
    // Make sure only a single OZEETY process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. OZEETY Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (GetBoolArg("-resync", false)) {
            ...
            fs::path eshadowDir = GetDataDir() / "eshadow";

            LogPrintf("Deleting blockchain folders blocks, chainstate, sporks and eshadow\n");
            ...
            try {
                ...
                if (fs::exists(eshadowDir)){
                    fs::remove_all(eshadowDir);
                    LogPrintf("-resync: folder deleted: %s\n", eshadowDir.string().c_str());
                }
            } catch (const fs::filesystem_error& error) {
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //OZEETY specific: eshadow and spork DB's
                eshadowDB = new CEshadowDB(0, false, fReindex);
                ...
                // OZEETY: load previous sessions sporks if we have them.
                ...
                bool fReindexEshadow = GetBoolArg("-reindexeshadow", false);
                ...
                int chainHeight;
                {
                    ...
                    // initialize OZTG and eZEE supply to 0
                    mapEshadowSupply.clear();
                    for (auto& denom : libeshadow::eshadowDenomList) mapEshadowSupply.insert(std::make_pair(denom, 0));
                    nMoneySupply = 0;

                    // Load OZTG and eZEE supply from DB
                    if (chainHeight >= 0) {
                        const uint256& tipHash = chainActive[chainHeight]->GetBlockHash();
                        CLegacyBlockIndex bi;

                        // Load eZEE supply map
                        if (!fReindexEshadow && consensus.NetworkUpgradeActive(chainHeight, Consensus::UPGRADE_ZC) &&
                                !eshadowDB->ReadZCSupply(mapEshadowSupply)) {
                            // try first reading legacy block index from DB
                            if (pblocktree->ReadLegacyBlockIndex(tipHash, bi) && !bi.mapEshadowSupply.empty()) {
                                mapEshadowSupply = bi.mapEshadowSupply;
                            } else {
                                // reindex from disk
                                fReindexEshadow = true;
                            }
                        }

                        // Load OZTG supply amount
                ...
                // Drop all information from the eshadowDB and repopulate
                if (fReindexEshadow && consensus.NetworkUpgradeActive(chainHeight, Consensus::UPGRADE_ZC)) {
                    LOCK(cs_main);
                    uiInterface.InitMessage(_("Reindexing eshadow database..."));
                    std::string strError = ReindexEshadowDB();
                    if (strError != "") {
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip ezee if already reindexed
                    RecalculateOZTGSupply(1, fReindexEshadow);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of OZEETY Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart OZEETY Core to complete") << "\n";
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_OZEETY && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_OZEETY);
            }
        ...
        zwalletMain = new CeZEEWallet(pwalletMain);
        ...
        if (!zwalletMain->GetMasterSeed().IsNull()) {
            //Inititalize eZEEWallet
            uiInterface.InitMessage(_("Syncing eZEE wallet..."));

            //Load eshadow mint hashes to memory
            pwalletMain->ezeeTracker->Init();
