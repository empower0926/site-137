...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasEshadowSpendInputs()) {
        libeshadow::CoinSpend zcspend = wtx.HasEshadowPublicSpendInputs() ? EZEEModule::parseCoinSpend(wtx.vin[0]) : TxInToEshadowSpend(wtx.vin[0]);
        fZSpendFromMe = wallet->IsMyEshadowSpend(zcspend.getCoinSerialNumber());
    }
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (!wtx.HasEshadowSpendInputs() && !ExtractDestination(wtx.vout[1].scriptPubKey, address))
            return parts;

        if (wtx.HasEshadowSpendInputs() && (fZSpendFromMe || wallet->ezeeTracker->HasMintTx(hash))) {
            //eZEE stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeEZEE;
            sub.address = mapValue["eshadowmint"];
            sub.credit = 0;
        ...
        parts.append(sub);
    } else if (wtx.HasEshadowSpendInputs()) {
        //eshadow spend outputs
        bool fFeeAssigned = false;
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
            const CTxOut& txout = wtx.vout[nOut];
            // change that was reminted as eshadows
            if (txout.IsEshadowMint()) {
                ...
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::EshadowSpend_Change_eZee;
                sub.address = mapValue["eshadowmint"];
                if (!fFeeAssigned) {
                    sub.debit -= (wtx.GetEshadowSpent() - wtx.GetValueOut());
                    fFeeAssigned = true;
                }
                sub.idx = nOut;
                parts.append(sub);
            ...
            // a eshadowspend that was sent to an address held by this wallet
            isminetype mine = wallet->IsMine(txout);
            if (mine) {
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                if (fZSpendFromMe) {
                    sub.type = TransactionRecord::EshadowSpend_FromMe;
                } else {
                    sub.type = TransactionRecord::RecvFromEshadowSpend;
                    sub.credit = txout.nValue;
                }
                sub.address = mapValue["recveshadowspend"];
                if (strAddress != "")
            ...
            // eshadow spend that was sent to someone else
            TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            sub.debit = -txout.nValue;
            sub.type = TransactionRecord::EshadowSpend;
            sub.address = mapValue["eshadowspend"];
            if (strAddress != "")
                sub.address = strAddress;
            sub.idx = nOut;
            parts.append(sub);
        ...
        if (fAllFromMe && fAllToMe) {
        ...
        } else if (fAllFromMe || wtx.HasEshadowMintOutputs()) {
            //
            // Debit
            //
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address)) {
                    ...
                    if (wtx.HasEshadowMintOutputs())
                        continue;
                    // Sent to OZEETY Address
                    sub.type = TransactionRecord::SendToAddress;
                    sub.address = EncodeDestination(address);
                } else if (txout.IsEshadowMint()){
                    sub.type = TransactionRecord::EshadowMint;
                    sub.address = mapValue["eshadowmint"];
                    sub.credit += txout.nValue;
                } else {
...
bool IsEZEEType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeEZEE:
        case TransactionRecord::EshadowMint:
        case TransactionRecord::EshadowSpend:
        case TransactionRecord::RecvFromEshadowSpend:
        case TransactionRecord::EshadowSpend_Change_eZee:
        case TransactionRecord::EshadowSpend_FromMe:
            return true;
        default:
            return false;
    }
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeEZEE ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeEZEE);
}
