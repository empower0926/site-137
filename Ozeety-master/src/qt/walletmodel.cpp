...
WalletModel::WalletModel(CWallet* wallet, OptionsModel* optionsModel, QObject* parent) : QObject(parent), wallet(wallet), optionsModel(optionsModel), addressTableModel(0),
                                                                                         ...
                                                                                         cachedEshadowBalance(0), cachedUnconfirmedEshadowBalance(0), cachedImmatureEshadowBalance(0),
...
CAmount WalletModel::getEshadowBalance() const
{
    return wallet->GetEshadowBalance(false);
}

CAmount WalletModel::getUnconfirmedEshadowBalance() const
{
    return wallet->GetUnconfirmedEshadowBalance();
}

CAmount WalletModel::getImmatureEshadowBalance() const
{
    return wallet->GetImmatureEshadowBalance();
}
...
void WalletModel::emitBalanceChanged()
{
    ...
    Q_EMIT balanceChanged(cachedBalance, cachedUnconfirmedBalance, cachedImmatureBalance,
                        cachedEshadowBalance, cachedUnconfirmedEshadowBalance, cachedImmatureEshadowBalance,
...
void WalletModel::checkBalanceChanged()
{
    ...
    CAmount newEshadowBalance = getEshadowBalance();
    CAmount newUnconfirmedEshadowBalance = getUnconfirmedEshadowBalance();
    CAmount newImmatureEshadowBalance = getImmatureEshadowBalance();
    ...
    if (cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance ||
        cachedEshadowBalance != newEshadowBalance || cachedUnconfirmedEshadowBalance != newUnconfirmedEshadowBalance || cachedImmatureEshadowBalance != newImmatureEshadowBalance ||
        ...
        cachedEshadowBalance = newEshadowBalance;
        cachedUnconfirmedEshadowBalance = newUnconfirmedEshadowBalance;
        cachedImmatureEshadowBalance = newImmatureEshadowBalance;
        ...
        Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance,
                            newEshadowBalance, newUnconfirmedEshadowBalance, newImmatureEshadowBalance,
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ozeety address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ozeety:URI (ozeety:XyZ...?message=example)
            {
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (isStaking)
        return error("Invalid OZEETY address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CTxDestination& address, CKeyID& keyID)
{
    if (!IsValidDestination(address))
        return error("Invalid OZEETY address");

    const CKeyID* inKeyID = boost::get<CKeyID>(&address);
    if (!inKeyID)
        return error("Unable to get KeyID from OZEETY address");
...
// returns a COutPoint of 10000 OZTG if found
