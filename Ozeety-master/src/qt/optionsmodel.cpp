...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::OZTG);
    ...
    if (settings.contains("nAnonymizeOzeetyAmount") || reset)
        SoftSetArg("-anonymizeozeetyamount", settings.value("nAnonymizeOzeetyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ozeety.cpp during shotdown to also remove the window positions
