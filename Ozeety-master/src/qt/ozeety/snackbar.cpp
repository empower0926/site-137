...
#include "qt/ozeety/forms/ui_snackbar.h"
#include "qt/ozeety/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(OZEETYGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &OZEETYGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
