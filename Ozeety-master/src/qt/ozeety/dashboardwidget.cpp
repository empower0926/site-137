...
#include "qt/ozeety/dashboardwidget.h"
#include "qt/ozeety/forms/ui_dashboardwidget.h"
#include "qt/ozeety/sendconfirmdialog.h"
#include "qt/ozeety/txrow.h"
...
DashboardWidget::DashboardWidget(OZEETYGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareOztg, "square-chart-oztg");
    setCssProperty(ui->labelSquareeZee, "square-chart-ezee");
    setCssProperty(ui->labelOztg, "text-chart-oztg");
    setCssProperty(ui->labelEzee, "text-chart-ezee");
    ...
    setCssProperty(ui->labelAmountOztg, "text-stake-oztg-disable");
    setCssProperty(ui->labelAmountEzee, "text-stake-ezee-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &OZEETYGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &OZEETYGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &OZEETYGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isOztg = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeEZEE;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair OZTG, eZEE
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            oztg = (pair.first != 0) ? pair.first / 100000000 : 0;
            ezee = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalOztg += pair.first;
            chartData->totalEzee += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesOztg.append(oztg);
        chartData->valueseZee.append(ezee);

        int max = std::max(oztg, ezee);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesOztg);
    set1->append(chartData->valueseZee);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalOztg > 0 || chartData->totalEzee > 0) {
        setCssProperty(ui->labelAmountOztg, "text-stake-oztg");
        setCssProperty(ui->labelAmountEzee, "text-stake-ezee");
    } else {
        setCssProperty(ui->labelAmountOztg, "text-stake-oztg-disable");
        setCssProperty(ui->labelAmountEzee, "text-stake-ezee-disable");
    }
    forceUpdateStyle({ui->labelAmountOztg, ui->labelAmountEzee});
    ui->labelAmountOztg->setText(GUIUtil::formatBalance(chartData->totalOztg, nDisplayUnit));
    ui->labelAmountEzee->setText(GUIUtil::formatBalance(chartData->totalEzee, nDisplayUnit, true));
