...
#include "qt/ozeety/topbar.h"
#include "qt/ozeety/forms/ui_topbar.h"
#include "qt/ozeety/lockunlock.h"
#include "qt/ozeety/qtutils.h"
#include "qt/ozeety/receivedialog.h"
#include "qt/ozeety/loadingdialog.h"
...
TopBar::TopBar(OZEETYGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopOztg}, "amount-small-topbar");
    setCssProperty({ui->labelAmountOztg}, "amount-topbar");
    setCssProperty({ui->labelPendingOztg, ui->labelImmatureOztg}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("OZEETY")
    updateDisplayUnit();
    ...
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        if (displayUnitPrev != nDisplayUnit)
            updateBalances(walletModel->getBalance(), walletModel->getUnconfirmedBalance(), walletModel->getImmatureBalance(),
                           walletModel->getEshadowBalance(), walletModel->getUnconfirmedEshadowBalance(), walletModel->getImmatureEshadowBalance(),
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
                            const CAmount& eshadowBalance, const CAmount& unconfirmedEshadowBalance, const CAmount& immatureEshadowBalance,
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // OZTG Total
    QString totalOztg = GUIUtil::formatBalance(balance, nDisplayUnit);

    // OZTG
    // Top
    ui->labelAmountTopOztg->setText(totalOztg);
    // Expanded
    ui->labelAmountOztg->setText(totalOztg);
    ui->labelPendingOztg->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureOztg->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
