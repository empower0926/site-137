...
#include "qt/ozeety/forms/ui_addresseswidget.h"
#include "qt/ozeety/addresslabelrow.h"
#include "qt/ozeety/addnewaddressdialog.h"
#include "qt/ozeety/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(OZEETYGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination oztgAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(oztgAdd, label.toUtf8().constData(),
