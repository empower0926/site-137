...
#include "qt/ozeety/forms/ui_send.h"
#include "qt/ozeety/addnewcontactdialog.h"
#include "qt/ozeety/qtutils.h"
#include "qt/ozeety/sendchangeaddressdialog.h"
#include "qt/ozeety/optionbutton.h"
#include "qt/ozeety/sendconfirmdialog.h"
#include "qt/ozeety/myaddressrow.h"
#include "qt/ozeety/guitransactionsutils.h"
...
SendWidget::SendWidget(OZEETYGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-oztg");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination oztgAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(oztgAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(oztgAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
