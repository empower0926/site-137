...
#include "qt/ozeety/splash.h"
#include "qt/ozeety/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ozeety-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ozeety.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ozeety.qrc)
...
/** Class encapsulating OZEETY Core startup and shutdown.
...
/** Main OZEETY application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (OZEETYGUI)
...
{
    window = new OZEETYGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &OZEETYGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &OZEETYGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(OZEETYGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(OZEETYGUI::DEFAULT_WALLET);
        ...
        // OZEETY: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OZEETYGUI::handlePaymentRequest);
        connect(window, &OZEETYGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. OZEETY can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ozeety_locale);
    Q_INIT_RESOURCE(ozeety);
    ...
    /// 6. Determine availability of data directory and parse ozeety.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("OZEETY Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("OZEETY Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ozeety: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("OZEETY Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
