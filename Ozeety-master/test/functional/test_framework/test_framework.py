...
TMPDIR_PREFIX = "ozeety_func_test_"


class OzeetyTestFramework():
    """Base class for a ozeety test script.

    Individual ozeety test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave ozeetyds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop ozeetyds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing ozeetyd/ozeety-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use ozeety-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: ozeetyds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a ozeetyd test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple ozeetyd test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'ozeetyd exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "ozeetyd should have exited with an error"
                else:
                    assert_msg = "ozeetyd should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as ozeetyd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in ozeety.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                for subdir in ["blocks", "chainstate", "sporks", "eshadow"]:
                    copy_and_overwrite(os.path.join(node_0_datadir, subdir),
                                    os.path.join(node_i_datadir, subdir))
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in ozeety.conf
        ...
        def clean_cache_subdir(cachedir):
            ...
            for i in range(MAX_NODES):
                for entry in os.listdir(cache_path(i)):
                    if entry not in ['wallet.dat', 'chainstate', 'blocks', 'sporks', 'eshadow', 'backups']:
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "ozeetyd"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run ozeetyds:
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one eshadow for each denom (tot 6666 OZTG) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    nBlocks += 1
                    # Mint eshadows with node-2 at block 301 and with node-3 at block 302
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 eshadows, one for each denom (tot 6666 OZTG), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 OZTG), change = 6750 - 6666 - fee
                        res.append(self.nodes[nBlocks-299].minteshadow(6666))
                        self.sync_all()
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 OZTG worth of eshadows
        ...
        # check eshadow balances / mints
        for peer in [2, 3]:
            if num_nodes > peer:
                zcBalance = self.nodes[peer].geteshadowbalance()
                zclist = self.nodes[peer].listmintedeshadows(True)
                zclist_spendable = self.nodes[peer].listmintedeshadows(True, True)
    ...
    def get_prevouts(self, node_id, utxo_list, zpos=False, nHeight=-1):
        ...
        :param   node_id:                   (int) index of the CTestNode used as rpc connection. Must own the utxos.
                 utxo_list: <if zpos=False> (JSON list) utxos returned from listunspent used as input
                            <if zpos=True>  (JSON list) mints returned from listmintedeshadows used as input
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for ezee-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For ezee prevScript is replaced with serialHash hex string.
        """
    ...
    def make_txes(self, node_id, spendingPrevOuts, to_pubKey):
        ...
        for uniqueness in spendingPrevOuts:
            if is_eshadow(uniqueness):
                # spend eZEE
                _, serialHash, _ = spendingPrevOuts[uniqueness]
                raw_spend = rpc_conn.createraweshadowspend(serialHash, "", False)
            else:
                # spend OZTG
    ...
    def stake_block(self, node_id,
        ...
        isZPoS = is_eshadow(block.prevoutStake)
        if isZPoS:
            # !TODO: remove me
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend eZEE inputs when fDoubleSpend is false. It needs to
                # be checked outside until a convenient tx.spends(eshadow) is added to the framework.
...
class ComparisonTestFramework(OzeetyTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some ozeetyd binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "ozeetyd"),
                          help="ozeetyd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "ozeetyd"),
                          help="ozeetyd binary to use for reference nodes (if any)")
