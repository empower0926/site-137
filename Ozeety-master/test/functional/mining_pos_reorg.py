...
from test_framework.test_framework import OzeetyTestFramework
...
class ReorgStakeTest(OzeetyTestFramework):

    def set_test_params(self):
        self.num_nodes = 3
        # node 0 and 1 stake the blocks, node 2 makes the eshadow spends
    ...
    def check_money_supply(self, expected_oztg, expected_ezee):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected OZTG and eZEE supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_oztg))
            for denom in node['eZEEsupply']:
                assert_equal(node['eZEEsupply'][denom], DecimalAmt(expected_ezee[denom]))
    ...
    def run_test(self):
        ...
        # Check OZTG and eZEE supply at the beginning
        # ------------------------------------------
        # eZEE supply: 2 coins for each denomination
        expected_ezee_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        self.check_money_supply(expected_money_supply, expected_ezee_supply)
        ...
        assert_equal(self.nodes[2].geteshadowbalance()['Total'], DecimalAmt(6666))
        self.log.info("Balances ok.")

        # create the raw eshadow spend txes
        addy = self.nodes[2].getnewaddress()
        self.log.info("Creating the raw eshadow public spends...")
        mints = self.nodes[2].listmintedeshadows(True, True)
        tx_A0 = self.nodes[2].createraweshadowspend(mints[0]["serial hash"], addy)
        tx_A1 = self.nodes[2].createraweshadowspend(mints[1]["serial hash"], addy)
        # Spending same coins to different recipients to get different txids
        new_addy = "yAVWM5urwaTyhiuFQHP2aP47rdZsLUG5PH"
        tx_B0 = self.nodes[2].createraweshadowspend(mints[0]["serial hash"], new_addy)
        tx_B1 = self.nodes[2].createraweshadowspend(mints[1]["serial hash"], new_addy)
        ...
        # Connect with node 2, sync and check eshadow balance
        self.log.info("Reconnecting node 0 and node 2")
        connect_nodes(self.nodes[0], 2)
        sync_blocks([self.nodes[i] for i in [0, 2]])
        self.log.info("Resetting eshadow mints on node 2")
        self.nodes[2].resetminteshadow(True)
        assert_equal(self.get_tot_balance(2), initial_balance[2] + DecimalAmt(minted_amount))
        assert_equal(self.nodes[2].geteshadowbalance()['Total'], DecimalAmt(6666-minted_amount))
        ...
        # Verify that OZTG and eZEE supplies were properly updated after the spends and reorgs
        self.log.info("Check OZTG and eZEE supply...")
        ...
        expected_ezee_supply[str(spent_coin_0)] -= spent_coin_0
        expected_ezee_supply[str(spent_coin_1)] -= spent_coin_1
        expected_ezee_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_ezee_supply)
