...
'''
Tests v2, v3 and v4 Eshadow Spends
'''
...
class EshadowSpendTest(OzeetyTestFramework):
    ...
    def log_title(self):
        ...
        underline = "-" * len(title)
        description = "Tests v2, v3 and v4 Eshadow Spends."
        self.log.info("\n\n%s\n%s\n%s\n", title, underline, description)

    def setV4SpendEnforcement(self, fEnable=True):
        sporkName = "SPORK_18_ESHADOW_PUBLICSPEND_V4"
    ...
    def run_test(self):
        ...
        def get_eshadow_data(coin):
            return coin["s"], coin["r"], coin["k"], coin["id"], coin["d"], coin["t"]

        def check_balances(denom, ezee_bal, oztg_bal):
            ezee_bal -= denom
            assert_equal(self.nodes[2].geteshadowbalance()['Total'], ezee_bal)
            oztg_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], oztg_bal)
            return ezee_bal, oztg_bal
        ...
        ezee_balance = self.nodes[2].geteshadowbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(ezee_balance, 6666)

        # Export eshadow data
        listmints = self.nodes[2].listmintedeshadows(True, True)
        serial_ids = [mint["serial hash"] for mint in listmints]
        exported_eshadows = [x for x in self.nodes[2].exporteshadows(False) if x["id"] in serial_ids]
        exported_eshadows.sort(key=lambda x: x["d"], reverse=False)
        assert_equal(8, len(exported_eshadows))

        # 1) stake more blocks - save a v3 spend for later (serial_1)
        serial_1, randomness_1, privkey_1, id_1, denom_1, tx_1 = get_eshadow_data(exported_eshadows[1])
        ...
        for j in range(5):
            for peer in range(2):
                for i in range(7):
                    block_time = self.generate_pos(peer, block_time)
                sync_blocks(self.nodes)
        old_spend_v3 = self.nodes[2].createraweshadowspend(id_1)

        # 2) Spend one minted coin - spend v3 (serial_2)
        serial_2, randomness_2, privkey_2, id_2, denom_2, tx_2 = get_eshadow_data(exported_eshadows[2])
        self.log.info("Spending the minted coin with serial %s..." % serial_2[:16])
        txid = self.nodes[2].spendeshadowmints([id_2])['txid']
        ...
        ezee_balance, balance = check_balances(denom_2, ezee_balance, balance)
        ...
        assert_raises_rpc_error(-4, "Trying to spend an already spent serial",
                                self.nodes[2].spendraweshadow, serial_2, randomness_2, denom_2, privkey_2, "", tx_2)
        ...
        serial_3, randomness_3, privkey_3, id_3, denom_3, tx_3 = get_eshadow_data(exported_eshadows[3])
        self.log.info("Spending the minted coin with serial %s..." % serial_3[:16])
        txid = self.nodes[2].spendeshadowmints([id_3])['txid']
        # stake 4 blocks - check it gets included on chain and check balances
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        ezee_balance, balance = check_balances(denom_3, ezee_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v4) PASSED")

        # 6) Check double spends - spend v4
        self.log.info("Trying to spend the serial twice now...")
        assert_raises_rpc_error(-4, "Trying to spend an already spent serial",
                                self.nodes[2].spendraweshadow, serial_3, randomness_3, denom_3, privkey_3, "", tx_3)
        ...
        self.nodes[2].resetminteshadow()
        _, _ = check_balances(denom_1, ezee_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")


if __name__ == '__main__':
    EshadowSpendTest().main()
